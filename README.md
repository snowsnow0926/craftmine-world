...
**本地创作 Alpha 0.5：真实模型编写新玩法源码，后台检查后进入世界。** 保留细粒度花草、对象记忆和基础战斗；新增独立执行的代码玩法、E 互动、物品计数与玩法状态保存。已经通过真实生成的滑门、弹跳板和多轮外观修复验收。
...
- **素材与记忆**：应用成功后自动记住对象和内置玩法配置。选择历史版本 → “复用到世界” → 应用候选，不需要再请求模型。LLM 在下一次创作时也会检索真实模块定义，开发记录显示读取与引用了哪些记忆。
...
- 新玩法源码与开关、冷却、物品等进度随完整世界存档保存；独立源码模块的记忆、依赖和跨世界复用进入下一阶段 M2。当前复用对象卡片会复用几何与组件。
