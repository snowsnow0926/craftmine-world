...
**本地创作 Alpha 0.4：细粒度花草、创作记忆库与基础玩法模块。** 由真实 LLM 生成场景和支持的玩法配置，应用成功后自动保存为可复用模块。任意玩法脚本、外部精细模型与社区联机仍待开发。
...
- **素材与记忆**：应用成功后自动记住对象和玩法。选择历史版本 → “复用到世界” → 应用候选，不需要再请求模型。LLM 在下一次创作时也会检索真实模块定义，开发记录显示读取与引用了哪些记忆。
...
- 玩法模块保存可运行的配置定义，具体系统代码在 `app/gameplay.mjs` 中。现阶段 LLM 能组合这些能力，不能自行添加任意 JS 系统。
...
- 右上角“导出存档”导出当前场景、最新位置和兼容的生命值／弹药／目标状态。完整存档不包含所有历史模块；可在记忆库中单独导出模块。
...
整体方向继续保留开发、游玩、素材三个工作区。多人共创、任意玩法代码开发、外部素材生成和社区发布属于后续阶段。
