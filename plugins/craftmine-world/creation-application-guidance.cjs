...
function creationApplicationGuidance(worldId,target,job=null,context=null){
 ...
 const result={format:'craftmine.creation-application-guidance/1',worldId,owner,captureSnapshotId:fullAuto?target.snapshotId:null,authority:'current-host-capture',grantsAuthority:false,mode:fullAuto?'full-auto':'manual-or-unavailable',
  instruction:'Describe the player-visible result and remaining real checks in ordinary language. Source files, build IDs and technical panels are not steps a full-auto player must perform. A passed check alone never proves adoption or every requested gameplay behavior. This receipt records an observed state; current result-card progress may advance before the player reads the final reply. Do not infer current invisibility from a pending receipt.'};
 if(!job)return {...result,adoptionConfirmed:false,playerActionRequired:fullAuto?false:null,nextAction:fullAuto?'complete-edit-check-then-finish-turn':'follow-current-authorization',
  instruction:result.instruction+(fullAuto?' The host coordinates application when verified content and authoring guards permit it. Turn-related steps refer to host authoring coordination, not the timing of the final reply on screen. If the matching receipt is deferred for CREATION_TURN_BUSY or CREATION_AWAITING_TURN_FINISH, finish authoring once the requested work is checked; do not wait for your own turn to end, ask for manual adoption, or claim it is already adopted.':' Do not infer automatic adoption from this guide; honor the actual application receipt and current player confirmation.')};
 ...
 if(fullAuto&&application.status==='deferred'){
  ...
  return {...result,...identity,adoptionConfirmed:false,playerActionRequired:false,timing:turn?'after-current-turn-settles':'when-owner-world-is-available',
   ...
   playerMessage:turn?'检查通过，已交给自动应用；最新进度见创作结果卡，无需额外操作。':'检查通过，自动应用的最新进度见创作结果卡，无需手动采用。',
   instruction:result.instruction+' This is an authorized automatic handoff, not a request for player confirmation. Keep this receipt\'s deferred status truthful; do not say adopted without newer evidence or tell the player to visit a technical panel. Turn-related timing describes host authoring coordination, not a guarantee that application waits for the visible conversation to end. If all requested edits are included in this last check, finish authoring and give a concise result with current progress in the result card. Further edits require a fresh check; this receipt is not a permanent promise after edits, cancellation, world changes or permission changes.'};
 }
