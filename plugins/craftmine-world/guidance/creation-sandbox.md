...
`state_guard.gd`、`component_state.gd`、`scene_mesh_picker.gd` 及 `project.godot` 中的运行时桥接入口。不能改弱或绕过这些检查；
...
当需要核对当前外观时，可调用 `godot_view_capture` 获取真实游戏图片。正式世界使用
`godot_runtime_state` 返回的真实 buildId/instanceId；已由玩家打开的候选预览使用确切
candidateId/buildId，候选 instanceId 可省略，由主机解析并在回执返回。不能从正式世界的
instanceId 猜候选身份，也不能仅用源码截图代替实际运行画面。
该工具不会打开预览、操作游戏或改变窗口。没有可见的已挂接游戏视图、身份变化，或当前
模型声明不支持图片时会明确不可用；按实际原因处理，不宣称已经看图。成功时图片位于
真正的 image block，文字回执说明世界/实例、捕获时间、原图与输出尺寸及 SHA-256。
模型收到图片和服务端是否接受图片仍是两层证据；仅有回执不能证明 provider 的视觉支持。
看图只能核对可见的外形和布局，碰撞、交互、进度和保存仍须实际运行验证。缩图只作用于
捕获后的图像，不能据此认定游戏窗口或视角改变。该能力不会修改托管拾取器。
...
刚性分件节点动画按当前实际变换参与精确网格拾取；AnimationPlayer 本身不是渲染几何。
...
库中若有可玩宠物组件，先读取实际声明，区分带跟随/互动/状态的模块和只提供犬外形的 GLB。
正常安装为每只伙伴生成独立根 `entity_id`。对同一只伙伴改名字、换外观，应修改当前实例
上的真实导出设置，保留根身份和其他实例；不要删除旧根重装一个新身份，也不要修改共享
GLB 来假装只改变一只。模型必须读取当前模块/场景确认参数，不能假定任意动物都支持犬模块。

托管造物世界支持可选 `body.components` 账本。组件自己的 `snapshot/validate_state/restore`
负责有版本的完整状态，`settings` 是运行设置，`sourceSettings` 是该实例源码初始设置。
正常候选迁移只把明确改过的源码设置更新到旧存档，保留位置、朝向、互动次数等未改进度；
不得直接改存档、把整个进度替换成默认值，或在 restore 里悄悄改输入使检查通过。
校验方法必须纯读，恢复方法要完整；恢复后若新形体进入障碍，应该修实际布置或实现，
不能缩小碰撞或删掉校验来掩盖穿模。新增实例从真实候选默认状态开始，移除实例的旧账本保留。

普通互动和后台有限 `interact` 走同一个真实准星命中与近距/遮挡路径。不要给所有附近宠物
广播按键、在初始化捕获鼠标，或靠瞬移追上玩家来冒充跟随。动画、碰撞、同身份换外观、
保存重开需分别核对真实结果。旧源码若缺少当前托管组件接口，走正常观察器升级/兼容流程，
不可用普通 patch 覆盖受保护适配器。
...
`deferred` 表示主机将待正确世界空闲后自动续作，`repairing` 表示已接续普通模型修复，均不需要玩家手动采用；
...
检查失败时根据实际错误继续修复和重检，不修改冻结断言，不把排队/阻塞解释为成功。不要在检查仍运行时以“请玩家检查/应用”结束全自动任务。
普通项目愿望即使没有有限词典对应断言，也应完成真实功能和视觉检查。宿主的通用运行、画面、存档与恢复检查通过只证明这些边界，不代表任意自然语言需求的语义验收已经完成。
