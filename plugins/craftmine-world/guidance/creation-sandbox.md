...
指导 ID：`creation-sandbox.authoring`，版本 `1.7.1`。仅匹配
...
在完整 v1/v2 组合中，物理碰撞关闭不等于视觉选择关闭。某模块的 `solid=false`
可以禁用其碰撞形状，同时保留可见网格；当前受支持的静态网格仍可能由三角形射线拾取。
以同一有效 sample 的 `sceneObjectSelection`、`sceneObjectTarget`、`sceneObjectRefs`
及其 ancestors 判断本次命中与所属模块，不能由“可穿过”推断“无法瞄准或选择”。
物理优先命中时 selection 的 `blocked` 也不能单独解释为不可选，要同时读取实际 target；
`hit` 只证明该次观察的受支持表面命中，范围仍是 base-surface-arrays，
不证明实际渲染 LOD 或逐像素遮挡精准，也不保证换视角后仍命中。

参数倍数和节点 position 不能单独证明物体的世界边界。没有实际网格/碰撞 bounds、
完整父子变换及相应测量时，“放大后越过 x=0”“挡住另一栋”均为 unknown，不能按印象推算。
`playerBounds` 是玩家胶囊边界，不是被选物体边界；一个射线命中点也不是完整物体范围。
...
`runtime.frame` 的 `frames` 是本次 probe 捕获的帧数，`distinct` 是其中不同图像 SHA-256
的数量。它只描述这一次 probe 内采样差异；两次检查的 distinct 从 1 变成 2，
不是跨版本画面对比，也不证明模型、尺寸或材质改对。静态场景的帧可完全相同；
该数量不作为画面变化或玩法成功的验收。跨版本外观结论需可比相机/场景条件下的真实图像或
几何测量及准确 build 身份；本段说明本身不提供这些证据。
