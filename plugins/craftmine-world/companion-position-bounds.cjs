...
const {rootBindingMatches,pinnedScripts,projectBindingMatches}=require('./companion-root-binding.mjs');

function companionPositionConfiguration(files,source){
  ...
  const known=applicable.find(profile=>files.get(profile.path)?.sha256===profile.sha256&&pinnedScripts(profile,files)&&profile.selectors.every(item=>item.path==='project.godot'?projectBindingMatches(profile,files,source):files.get(item.path)?.sha256===item.sha256||item.path===profile.rootBinding?.scene&&rootBindingMatches(profile,files)));
