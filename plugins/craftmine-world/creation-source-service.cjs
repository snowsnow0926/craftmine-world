...
function createCreationSourceService({core,capture,sample,assertActive}) {
 ...
 return async function execute({context,workspace,request}){
  ...
  const sourceIndex=await index(context,workspace.worldId,version);
  ...
  for(const path of [SCENE_PATH,JOURNAL_PATH])if(sourceIndex.files.has(path))files[path]=await read(context,workspace.worldId,sourceIndex,sourceIndex.files.get(path));
  ...
  const previous=async()=>core.call('godotProject.receipt',{binding:workspace.task.binding,worldId:workspace.worldId,toolCallId,method:'godotProject.patch',request:params});
  const response=(receipt,replayed)=>({format:'craftmine.creation-operation-result/1',receipt:result.receipt,source:{revision:receipt.revision,manifestHash:receipt.manifestHash},replayed,applied:false,checkRequired:true});
  ...
  let receipt;
  try{assertActive(context);receipt=await core.call('godotProject.patch',params);}catch(error){try{receipt=await previous();}catch{}if(!receipt)throw error;}
