...
Every field name is strict. Assertion explanations are why and red (both strings); redNote, description and expectedResult are not supported aliases. Do not add fields outside the assertion guide or wrap the object in commentary.
...
Steps are data, not scripts: {label:string,event:{type:"tick"|"interact"|"contact"|"attack"|"land"|"key"|"equip"|"reload",targetId?:string,code?:string,weapon?:"ranged"|"melee"},dt?:0..1,player?:{x,y,z,yaw?,pitch?}}. weapon is only for equip; equip/reload have no targetId. Attack follows the actual gameplay attack path and aiming ray, so targetId alone does not establish a hit. Maximum 24 steps. The candidate starts once before steps; do not add a start step. Only use keys declared by the authored behavior. Player coordinates and optional angles are bounded test setup, never OS input. These events do not simulate walking physics, graphics recognition or sound perception. Explicitly list unverified aspects and omitted attachments under limitations. Static object creation may use an empty steps array.
