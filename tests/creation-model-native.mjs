...
const until=async(read,accept,label,timeout=90000)=>{const deadline=Date.now()+timeout;let value;while(Date.now()<deadline){if(ended)throw Error('Electron exited');value=await read();if(accept(value))return value;await delay(500);}throw Error(label+' timed out');};
...
async function restart(){await stop();start();await started();await nav('world.open',{id:worldId});await settled(worldId);const initialized=await evaluate('initialize');assert.equal(initialized.sessionId,sessionId);assert.equal(initialized.reopened,true);}
