...
const report=await run('encounter',commands);const attacks=report.results.filter(x=>x.op==='attack').map(x=>x.result);assert.ok(attacks.some(x=>x.hits?.some(h=>h.damage>0)),'真实射线必须命中怪物');assert.ok(attacks.at(-1).hits.some(h=>h.damage>0));const interaction=report.results.find(x=>x.op==='interact')?.result;assert.equal(interaction?.handled,true);assert.deepEqual(interaction.items,['coin','fang']);assert.equal(report.results.find(x=>x.op==='save')?.result?.written,true);
