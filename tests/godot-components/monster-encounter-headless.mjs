...
commands.splice(commands.findIndex(x=>x.op==='interact'),1,{op:'test-set-inventory-capacity',args:{capacity:1}},{op:'interact'},{op:'interact'},{op:'inventory-remove',args:{id:'repair_kit'}},{op:'test-set-inventory-capacity',args:{capacity:2}},{op:'interact'});const report=await run('encounter',commands);const attacks=report.results.filter(x=>x.op==='attack').map(x=>x.result);assert.ok(attacks.some(x=>x.hits?.some(h=>h.damage>0)),'真实射线必须命中怪物');assert.ok(attacks.at(-1).hits.some(h=>h.damage>0));const interactions=report.results.filter(x=>x.op==='interact').map(x=>x.result);assert.equal(interactions[0]?.handled,false,'容量不足不得领取');assert.equal(interactions[1]?.handled,false,'容量不足重复领取不得改变状态');assert.equal(interactions[2]?.handled,true);assert.deepEqual(interactions[2].items,['coin','fang']);const saved=report.results.find(x=>x.op==='save')?.result; assert.equal(saved?.written,true);
