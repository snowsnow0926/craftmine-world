...
host.resolveModuleNames=(names,containing)=>names.map(name=>ts.resolveModuleName(name,containing.replace(current,dependencyRoot),options,ts.sys).resolvedModule);
const program=ts.createProgram([current+'/vendor/pi-desktop/apps/desktop/electron/main/godot-world-view-host.ts'],options,host);
