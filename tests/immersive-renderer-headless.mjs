...
import {openCraftmineModeEntry} from './src/lib/craftmine-mode';
...
globalThis.fixture={
 ...
 state:()=>({nativeBounds,immersion,presentations,activeTab:useAppStore.getState().activeWorkPanelTabId,session:useAppStore.getState().activeSessionId,running:useAppStore.getState().isRunning}),
...
try {
  ...
  await page.waitForFunction(()=>document.querySelector('[data-mode-entry]')&&!document.querySelector('[data-testid="startup-splash"]'));
  check('startup offers equal side-by-side primary modes before opening a world',await page.evaluate(()=>{const a=document.querySelector('[data-mode="play"]').getBoundingClientRect(),b=document.querySelector('[data-mode="create"]').getBoundingClientRect();return a.width>0&&Math.abs(a.width-b.width)<1&&a.right<=b.left&&Math.abs(a.y-b.y)<1&&!document.querySelector('.work-plugin-view-surface');}));
  ...
  for(const overlay of ['closed','compact','full','closed','compact']) {
    ...
    await page.waitForFunction(()=>{const a=fixture.state().nativeBounds;return a&&Math.abs(a.x)<1&&Math.abs(a.y)<1&&Math.abs(a.width-innerWidth)<1&&Math.abs(a.height-innerHeight)<1;});
    check(overlay+' preserves full-client world geometry',true);
    if(overlay!=='closed') check(overlay+' floats centered over the world',await page.evaluate(()=>{const b=document.querySelector('.main-pane').getBoundingClientRect();return Math.abs(b.x+b.width/2-innerWidth/2)<1&&Math.abs(b.y+b.height/2-innerHeight/2)<1&&b.width<innerWidth&&b.height<innerHeight;}));
    ...
    if(overlay==='compact') await page.screenshot({path:path.join(output,'compact.png')});
  }
  ...
  check('picker extending above compact is included in overlay measurements without resizing world',true);
