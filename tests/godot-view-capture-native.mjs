...
try{
 ...
 record.movement=await rpc('godotExplore',{payload:{...formal,steps:[{op:'look',args:{yaw:0.6,pitch:-0.2},capture:false},{op:'walk',args:{forward:1,right:0,frames:36},capture:false},{op:'wait',args:{frames:12},capture:false}]}});
 ...
 assert.equal(record.latestObservation.instanceId,formal.instanceId);assert.deepEqual(record.latestSnapshot.body.player,record.latestObservation.payload.player);assert.notDeepEqual(record.latestSnapshot.body.player.position,record.beforeMovement.world.snapshot.body.player.position);
