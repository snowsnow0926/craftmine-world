...
fs.mkdirSync(path.join(root,'test-results'),{recursive:true});const out=fs.mkdtempSync(path.join(root,'test-results/batch07-ui-'));
...
try{
  ...
  await context.addInitScript(record=>{
    ...
    const channels=['workbench.operations','workbench.prepare','workbench.execute','workbench.acknowledge','draft.recheck','task.budget','library.search','library.read','library.install','library.capture','memory.search','memory.propose','memory.retire','task.current','task.recoverable','task.resume','task.discard','task.stop','backup.export','backup.inspect','backup.restore','backup.status','diagnostics.status','diagnostics.export','selection.set','selection.clear'];
    globalThis.pluginBridge={on(){},async invoke(channel,args={}){
      ...
      const operations=JSON.parse(localStorage.getItem('fixture-pending')||'[]'),store=()=>localStorage.setItem('fixture-pending',JSON.stringify(operations));
      if(channel==='workbench.operations')return {items:operations.filter(item=>item.worldId===args.worldId)};
      if(channel==='workbench.prepare'){let prior=operations.find(item=>item.worldId===args.worldId&&item.channel===args.channel&&JSON.stringify(item.payload)===JSON.stringify(args.payload));if(!prior){prior={worldId:args.worldId,operationId:crypto.randomUUID(),channel:args.channel,payload:args.payload,state:'pending'};operations.push(prior);store();}return prior;}
      if(channel==='workbench.execute'){const item=operations.find(item=>item.worldId===args.worldId&&item.operationId===args.operationId);if(!item)throw Error('OWNER_MISMATCH');if(item.state==='completed')return item.result;try{item.result=await pluginBridge.invoke(item.channel,{worldId:item.worldId,...item.payload,operationId:item.operationId});item.state='completed';store();return item.result;}catch(error){item.state='uncertain';store();throw error;}}
      if(channel==='workbench.acknowledge'){const index=operations.findIndex(item=>item.worldId===args.worldId&&item.operationId===args.operationId&&item.state==='completed');if(index<0)throw Error('UNCONFIRMED');operations.splice(index,1);store();return {acknowledged:true};}
      if(channel==='draft.recheck')return {verificationId:'check-fixture',status:'queued',applied:false};
      if(channel==='task.budget'){state.limit=args.maxTokens;return {operationId:args.operationId,budget:{limits:{maxTokens:args.maxTokens}}};}
      ...
      if(channel==='task.current')return {active:state.active,context:{binding:{taskId:'task-a'},generation:1,status:state.active?'running':'finished',draft:{revision:3,hash:'b'.repeat(64)},requirements:[{kind:'correction',text:'花草可以穿行'}],modifiedResources:['object:flower'],budget:{requestCount:4,compactionCount:3,actualTokens:1000,reservedTokens:200,unknownRequestCount:1,remainingTokens:8800,limits:{maxRequests:80,maxCompactions:8,maxTokens:state.limit===undefined?10000:state.limit}}}};
  ...
  await submitByText('确认已完成');
  await page.evaluate(()=>{__state.loseInstallReply=true;});await submitByText('加入当前草稿');
  const unresolved=await page.evaluate(()=>JSON.parse(localStorage.getItem('fixture-pending'))[0].operationId);
  await page.reload();await page.waitForFunction(()=>document.body.dataset.worldLoaded==='true');await open('task');
  check('renderer restart shows saved unresolved operation without automatic execution',await page.evaluate(id=>document.querySelector('[data-pending-operation="'+id+'"]')!==null&&!__state.calls.some(c=>c.channel==='library.install'),unresolved));
  await submitByText('查询并继续原操作');check('explicit recovery executes the original saved operation',await page.evaluate(id=>__state.calls.some(c=>c.channel==='library.install'&&c.args.operationId===id),unresolved));
  await submitByText('确认已完成');await open('library');
  ...
  await submitByText('解除本地累计 token 上限');check('budget setting uses a player operation and distinguishes cumulative limit',await page.evaluate(()=>__state.calls.some(c=>c.channel==='task.budget'&&c.args.maxTokens===null)&&document.querySelector('[data-workbench-page="task"]').textContent.includes('不是模型一次能读取')));
  await submitByText('确认已完成');await submitByText('重新检查已保存草稿');check('recheck keeps captured draft hash revision and generation',await page.evaluate(()=>__state.calls.some(c=>c.channel==='draft.recheck'&&c.args.revision===3&&c.args.draftHash==='b'.repeat(64)&&c.args.generation===1)));
  await submitByText('确认已完成');await open('memory');check('memory shows world scope and source',await page.locator('[data-memory-id]').textContent().then(t=>t.includes('仅此世界')&&t.includes('user:request-1')));
  ...
  await submitByText('确认已完成');await open('backup');await submitByText('选择备份并查看内容');check('restore explicitly describes all profile worlds',await page.locator('[data-backup-inspection]').textContent().then(t=>t.includes('全部世界资料')&&t.includes('世界 2')));
  ...
  check('unknown restore can query existing operation receipt',await page.getByText('查询并继续原操作',{exact:true}).count()===1);
