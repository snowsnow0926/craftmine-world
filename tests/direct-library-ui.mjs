...
if(args.action==='inspect'){if(f.holdInspect)await new Promise(resolve=>f.releaseInspect=resolve);return{eligible:args.ref.assetId!=='unsupported',reason:args.ref.assetId==='unsupported'?(f.inspectionReason??'SOURCE_PACKAGE_UNSUPPORTED'):undefined,compatibility:'unchecked',positionSupported:true};}
...
f.validateResponses=()=>{const attempt=currentDirectAttempts()[0],op=f.ops.get(attempt.request.operationId);return [{...op,worldId:'other'}, {...op,operationId:'other-operation'}, {...op,ref:{...op.ref,version:99}}, {...op,status:'success'}, {...op,modelCalls:1},{...op,checkProgress:{stage:'unknown',percent:50}},{...op,timings:{preparationMs:-1}}].every(value=>{try{parseDirectOperation(value,attempt);return false;}catch{return true;}});};
f.render();`;
...
const browser=await playwright().chromium.launch({...browserOptions(),args:['--allow-file-access-from-files','--disable-gpu']}),context=await browser.newContext({viewport:{width:1280,height:1000}}),page=await context.newPage();
...
await page.evaluate(()=>{const op=[...fixture.ops.values()].at(-1);op.checkProgress={stage:'import',percent:10};op.timings={preparationMs:1234,applyMs:567};op.updatedAt=++fixture.counter;});await submit('[data-direct-action="status"]');await page.waitForSelector('[data-direct-native-stage="import"]');
check('native progress and measured durations use real receipt values',await page.evaluate(()=>document.querySelector('[data-direct-native-stage]').textContent.includes('10%')&&document.querySelector('[data-direct-timings]').textContent.includes('1.2')&&document.querySelector('[data-direct-timings]').textContent.includes('0.6')));
...
await page.evaluate(()=>fixture.inspectionReason='PACKAGE_SINGLE_ENTITY_DECLARATION_REQUIRED');await select('unsupported');await page.waitForSelector('[data-direct-unavailable]');check('known broken version explains recovery, keeps details and emits no automatic model request',await page.evaluate(()=>document.querySelector('[data-direct-unavailable]').textContent.includes('该素材版本无法直接加入世界，请在素材库选择可用版本')&&document.body.textContent.includes('PACKAGE_SINGLE_ENTITY_DECLARATION_REQUIRED')&&!document.querySelector('[data-direct-start-form]')&&fixture.ai.length===0));check('host unsupported refusal preserves AI modification',await page.evaluate(()=>!!document.querySelector('[data-asset-use="modify"]')&&!document.querySelector('[data-direct-start-form]')));
...
check('profile persistence contains only exact request locators and labels, never cached receipts',await page.evaluate(()=>JSON.parse(localStorage.getItem('craftmine.direct-library.operations.v1')).every(row=>Object.keys(row).sort().join(',')==='displayName,request')&&!sessionStorage.getItem('craftmine.direct-library.operations.v1')));
await page.evaluate(()=>fixture.unmount());await page.waitForSelector('[data-closed]');
const reopened=await page.context().newPage();reopened.on('pageerror',error=>report.errors.push(String(error)));
await reopened.addInitScript(value=>{window.savedHostOps=value;window.violations=[];Element.prototype.requestPointerLock=function(){violations.push('pointerLock');throw Error('BLOCKED');};window.focus=()=>violations.push('focus');HTMLElement.prototype.focus=function(){};},beforeReload.host);
await reopened.goto(pathToFileURL(path.join(out,'index.html')).href);await reopened.waitForSelector('[data-asset-id]');await reopened.evaluate(()=>fixture.render('reload-world'));await reopened.waitForSelector('[data-direct-status="checking"]');
check('a new renderer page with empty session storage reconciles local operation locators without replay',await reopened.evaluate(id=>sessionStorage.length===0&&fixture.calls.some(c=>c.args.action==='status'&&c.args.operationId===id)&&!fixture.calls.some(c=>c.args.action==='start')&&violations.length===0,beforeReload.id));
