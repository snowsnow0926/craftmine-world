...
test('released cohorts and archived references stay unchanged while the guidance text has its own version',()=>{
 ...
 assert.equal(hash(JSON.stringify(skill.references.filter(ref=>ref.path!=='library/asset-index.json'))),'0ca22508585c004bba59e2074f8f4fcae9e910f9bfd8ade5c02076f71db767ee');
 assert.equal(skill.version,'1.9.0');
 assert.equal(hash(skill.text),'8d0066422e0033aa282fa3519700d406621e31a44fb62e1b842a11759f4b2924');
 assert.deepEqual(skill.interfaceCohorts,registry);assert.equal(corpus.version,'1.9.0');
});
