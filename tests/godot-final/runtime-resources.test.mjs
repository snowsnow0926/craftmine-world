...
async function fixture(){
  ...
  for(const relative of ['blender/runtime/blender.exe','git/bin/git.exe','godot/broker/godot-host-broker.exe','licenses/blender/README.md','licenses/godot/LICENSE.txt','licenses/gpl/GPL-3.0.txt']){
...
test('resource inventory hashes real bytes and verifies the exact source commit',async()=>{
  ...
  assert.equal((await verifyRuntimeResources(directory,'commit')).files.length,6);
...
test('packaged mode permits other app resources but detects extra files inside managed trees',async()=>{
  ...
  assert.equal((await verifyRuntimeResources(directory,'commit',{packaged:true})).files.length,6);
