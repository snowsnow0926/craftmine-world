...
try{
  ...
  for(const baseId of process.env.CRAFTMINE_TEST_BASES?.split(',')??['first-person','top-down','side-view']){
    const starterId=baseId==='first-person'?'training-range':baseId==='top-down'?'town':'ruins';
  ...
  for(const world of worlds){
    await step(`reopen saved ${world.baseId}`,async()=>{await nav('world.open',{id:world.id});await settled(world.id);const actual=await rpc('godotObserve');assert.equal(actual.worldId,world.id);assert.equal(actual.buildId,world.observation.buildId);await capture(world.baseId);return actual;});
  }
  ...
  if(process.env.CRAFTMINE_TEST_BACKUP==='1'){
    ...
    await step('restart restored data-root pointer',async()=>{await stop();start();await started();await nav('world.open',{id:observed.worldId});const fresh=await rpc('godotObserve');assert.equal(fresh.worldId,observed.worldId);assert.equal(fresh.buildId,observed.buildId);return capture(fresh.baseId);});
  }
