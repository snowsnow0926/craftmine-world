...
if(!process.argv.includes('--run')){
 ...
 console.log(JSON.stringify({mode:'prepare-only',role:'developer-arranged-prefab-demo',modelCalls:0,plan,packagedRoot,packages,needsFrozenPackage:!packagedRoot,...(resume?{sourceReport,worldId:prior.worldId,alreadyApplied:resume.completed.length,resumeFrom:resume.failed.assetId}: {})}));process.exit(0);
}
...
const out=sourceReport?path.dirname(sourceReport):createCompleteOutput(root),profile=path.join(out,'profile'),legacySource=path.join(out,'legacy'),runPrefix=resume?'resume-'+randomUUID()+'-':'';
let marker,token;if(resume){marker=fs.readFileSync(path.join(profile,'headless-profile.json'),'utf8');token=JSON.parse(marker).token;readHeadlessProfile({CRAFTMINE_HEADLESS_TEST:'1',CRAFTMINE_HEADLESS_ROOT:out,CRAFTMINE_DATA_DIR:profile,CRAFTMINE_HEADLESS_TOKEN:token});}
else{token=randomUUID();fs.mkdirSync(profile);fs.mkdirSync(legacySource);marker=JSON.stringify({format:'craftmine.headless-profile/1',token,legacySource});fs.writeFileSync(path.join(profile,'headless-profile.json'),marker);}
const proofs=sourceReport?[fileProof(sourceReport),fileProof(path.join(profile,'headless-profile.json'))]:[];
if(resume)for(const previous of resume.completed)assert.equal(packages[previous.index].archiveSha256,previous.archiveSha256,'Previously applied components must match the continuation plan');
const report={format:'craftmine.builtin-prefab-demo/1',role:'developer-arranged-prefab-demo',label:'开发者布置预制组件演示，非模型生成',packageIdentity:client.identity,startedAt:new Date().toISOString(),modelCalls:0,creationEvaluation:false,sourceEditsByHarness:0,plan,installations:resume?resume.completed.map(r=>({...r,alreadyAppliedInSourceReport:true})):[],launches:[],calls:[],visual:'UNVERIFIED',ok:false,...(resume?{sourceReport,worldId:prior.worldId,previousProductInventory:prior.packageIdentity.inventorySha256,initialSource:prior.initialSource,continuedAfterFailure:prior.error}: {})};
const output=path.join(out,runPrefix?runPrefix+'report.json':'report.json');fs.writeFileSync(output,JSON.stringify(report,null,2),{flag:'wx'});
const save=()=>fs.writeFileSync(output,JSON.stringify(report,null,2));
let worldId=prior?.worldId,child,ready=false,ended=true,exit,launch;const pending=new Map();
function start(){
 ...
 for(const stream of ['stdout','stderr'])child[stream].on('data',bytes=>{current[stream+'Bytes']+=bytes.length;fs.appendFileSync(path.join(out,runPrefix+current.number+'-'+stream+'.log'),bytes);});
...
try{
 ...
 report.before=await until(()=>rpc('godotObserve'),r=>r.worldId===worldId&&r.instanceId,'world runtime');await panel('godot.runtimeResume');report.beforeCapture=await capture('before.png');const currentSource=await pkg('sourceList');
 if(resume){assert.equal(report.before.buildId,resume.buildId,'Preserve the previously adopted environment');assert.equal(currentSource.revision,resume.source.revision);assert.equal(currentSource.manifestHash,resume.source.manifestHash);report.resumedSource=currentSource;}else report.initialSource=currentSource;
 for(const [index,item]of packages.entries()){
  if(index<(resume?.completed.length??0))continue;
  ...
  const operationId=randomUUID(),record={index,assetId:item.request.assetId,archiveSha256:item.archiveSha256,position:item.request.position??null,operationId,...(resume&&index===resume.failed.index?{previousFailedOperationId:resume.failed.operationId,retryReason:'original failure occurred before a source transaction; prior task was finalized'}:{})};report.installations.push(record);save();
...
finally{if(!ended)try{await stop();}catch(error){report.shutdownError=String(error);report.ok=false;process.exitCode=1;}try{assertProofs(proofs);}catch(error){report.integrityError=String(error);report.ok=false;process.exitCode=1;}report.endedAt=new Date().toISOString();save();console.log('Report: '+output);}
