...
import{requireLegacyTree,requireSameLegacySave,LEGACY_TREE_REQUEST,LEGACY_TREE_BASE}from'./helpers/legacy-tree-contract.mjs';
...
test('cold reopen requires complete saved progress and exact source build',()=>{
  const saved={record,snapshot:{player:{x:1,y:6,z:0},behaviors:{time:0}}};assert.equal(requireSameLegacySave(saved,structuredClone(saved)),true);
  const changed=structuredClone(saved);changed.snapshot.player.x=2;assert.throws(()=>requireSameLegacySave(saved,changed),/COLD_PROGRESS_CHANGED/);
});
