...
for(const grouped of [false,true])test(`real immutable catalog ${grouped?'group':'single'} proposal reaches one Godot draft transaction without legacy conversion`,{skip:!process.env.CRAFTMINE_CORE_BIN},async t=>{
 ...
 const files={'tree.gd':Buffer.from('extends Node3D\n@export var entity_id: String = ""\n'),'tree.gd.uid':Buffer.from('uid://b12345678901\n'),'tree.tscn':Buffer.from('[gd_scene load_steps=2 format=3]\n[ext_resource type="Script" path="res://addons/tree/tree.gd" id="1"]\n[node name="Tree" type="Node3D"]\nscript = ExtResource("1")\n')};
 const content={assetId:'tree',version:1,kind:'object',files:Object.entries(files).map(([path,bytes])=>({path,bytes:bytes.length,sha256:sha(bytes)})),dependencies:[],entry:{entities:['tree'],sceneInstall:{mode:'instance',sceneFile:'tree.tscn',identityField:'entity_id'}},interfaces:{},compatibility:{},state:{},licenses:{}};
