...
try {
  ...
  await page.exposeBinding('__craftmineBridge',(source,channel,payload)=>{
  ...
  check('保存接口不能用额外参数替换世界源码',(await bridge('world.saveProgress',{id:first.id,revision:persisted.revision,baseBuild:first.world.build.id,snapshot:persisted.world.snapshot,build:{id:'forged'}})).world.build.id===first.world.build.id);
