...
const browser=await playwright().chromium.launchPersistentContext(path.join(output,'profile'),{...browserOptions(),headless:true,viewport:{width:960,height:720}});
try{
 ...
 check('failed status remains visible with the host reason',await page.evaluate(()=>document.querySelector('.craftmine-overlay-task').title==='REQUIREMENT_SIZE_MISMATCH'));
