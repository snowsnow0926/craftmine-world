...
const methods={
 async bootstrap(){
  ...
  try{await probe.ensure({worldId:'alpha',buildId:'authored-bootstrap',root:exportRoot,artifacts:files(exportRoot),entry:'index.html',revision:0,snapshot:null,timeoutMs:90000});const result=await probe.snapshot();await seed(result.state);return {baseId:result.state.baseId,format:result.state.format};}finally{await probe.close();}
 },
