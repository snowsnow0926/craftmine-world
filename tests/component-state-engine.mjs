...
try{await engine.run('import',['--path',project,'--editor','--import'],{timeout:60000});const output=await engine.run('state',['--path',project,'--script','res://probe.gd','--quit-after','300'],{timeout:15000});const line=output.split(/\r?\n/).find(x=>x.startsWith('COMPONENT_STATE='));assert.ok(line,output);report.result=JSON.parse(line.slice('COMPONENT_STATE='.length));assert.equal(report.result.checks.length,26);report.passed=true;}finally{fs.writeFileSync(path.join(out,'report.json'),JSON.stringify(report,null,2));console.log(JSON.stringify(report));}
