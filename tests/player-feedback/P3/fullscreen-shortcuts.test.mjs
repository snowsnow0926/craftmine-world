...
test('editing context is retained until the next key',async()=>{const h=harness(),e={...event};h.setEditing(true);h.capture(e);h.setEditing(false);h.bubble(e);await tick();assert.equal(h.actions.length,0);h.dispose();});
