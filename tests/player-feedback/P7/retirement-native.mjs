...
const core=path.join(root,'test-results/core-target/debug/craftmine-core.exe');
const broker=path.join(root,'test-results/broker-target/release/godot-host-broker.exe');
const engineRoot='D:/Craftmine World/desktop/build/godot/4.7.2-stable';
const identity=path.join(out,'broker-identity.json');
fs.writeFileSync(identity,JSON.stringify({format:'craftmine.godot-broker-identity/1',sha256:hash(fs.readFileSync(broker)),bytes:fs.statSync(broker).size,sourceCommit,protocolVersion:1}));
