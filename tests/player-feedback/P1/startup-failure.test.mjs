...
async function fixture(mode){
 ...
 const context={module:{exports:{}},console,join,resolve,sep,realpath:fs.realpath,setTimeout,clearTimeout,setInterval,clearInterval,Date,Promise,WORLD_CHROME_HEIGHT:76,GODOT_WORLD_MESSAGE_CHANNEL:'message',GODOT_WORLD_DETACH_CHANNEL:'detach',
  NO_IMMERSION:{active:false,overlay:'closed',overlayBounds:null},createImmersionPauseController,hasHeadlessController:()=>false,syncMainInputFocus:()=>{},
 ...
 const host=new Host({window:()=>null,allowedRoots:()=>[root],onState:state=>states.push(state)});
 host.createView=()=>{
 ...
 return {host,states,request:{worldId:'alpha',buildId:'build-alpha',root,revision:0,snapshot:null,timeoutMs:80,artifacts:[{path:'index.html',bytes:bytes.length,sha256:createHash('sha256').update(bytes).digest('hex')}]},runtime:()=>runtime};
}
test('a load timeout reports its pre-cleanup cause, not the renderer destroyed by cleanup',async()=>{
 ...
 assert.deepEqual(f.states.filter(state=>state.state==='loading').map(state=>state.loadingStage),['resources','engine','scene']);
