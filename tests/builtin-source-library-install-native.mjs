...
try{
 ...
 const pet=inventory.entries.find(entry=>entry.assetId===PET_ASSET_ID);
 if(pet){
 ...
 for(const file of managed)assert.equal(sha(await readFile(file.path)),sha(Buffer.from(file.text)),'Managed component source requirement changed: '+file.path);
 const lock=JSON.parse((await readFile('craftmine.assets.lock.json')).toString());assert.equal(lock.assets.length,selectedEntries.length);
 ...
 report.limit='All selected built ZIPs reached real package plan + source transaction. Check is blocked because no executor is registered; this is not engine, runtime or visual acceptance.';
