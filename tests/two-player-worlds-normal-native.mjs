...
async function launch(label){
 ...
 try{
  ...
  const attached=async(kind,id)=>{const evidence=await until(async()=>{const views=await native(),main=views.pages.find(p=>p.url.includes('/out/renderer/index.html')),owner=views.windows.find(w=>w.children.some(child=>child.id===main?.id));if(!owner)return{ok:false};const game=await getProduct(),surface=await game.evaluate(()=>({id:document.body.dataset.worldId,loaded:document.body.dataset.worldLoaded==='true',godot:document.body.dataset.godot==='true',bounds:(()=>{const r=document.querySelector('iframe')?.getBoundingClientRect();return r?{width:r.width,height:r.height}:null;})()}));if(kind==='web'){const host=views.pages.find(p=>p.url.includes('/views/world.html')),child=owner.children.find(child=>child.id===host?.id);let canvas=null;for(const frame of game.frames().filter(frame=>frame!==game.mainFrame())){canvas=await frame.evaluate(()=>{const el=document.querySelector('canvas'),r=el?.getBoundingClientRect();return el&&r?{width:r.width,height:r.height,pixelWidth:el.width,pixelHeight:el.height}:null;}).catch(()=>null);if(canvas)break;}return{views,surface,canvas,child,ok:surface.id===id&&surface.loaded&&!surface.godot&&host&&!host.offscreen&&child?.bounds.width>0&&child?.bounds.height>0&&canvas?.width>0&&canvas?.height>0};}const bound=await rpc('godotCaptureBoundState'),formal=bound.formal;const matches=views.pages.filter(p=>p.scope&&['worldId','buildId','instanceId'].every(key=>p.scope[key]===formal?.[key]));const child=owner.children.find(child=>matches.some(p=>p.id===child.id));return{views,surface,bound,child,ok:surface.id===id&&surface.loaded&&formal?.worldId===id&&!bound.candidate&&matches.length===1&&!matches[0].offscreen&&child?.bounds.width>0&&child?.bounds.height>0};},value=>value.ok);report.attachments.push({kind,worldId:id,...evidence});write();return evidence;};
