...
async function launch(label){
 ...
 try{
  ...
  const prepareCancelRetry=async(webId)=>{
    await action('[data-player-world="godot"]');
    const observation=await until(()=>page.evaluate(()=>({cancel:!!document.querySelector('[data-player-world-cancel]:not(:disabled)'),entered:!document.querySelector('[data-mode-entry]'),error:document.querySelector('[data-player-world-error]')?.textContent??null})),value=>value.cancel||value.entered||value.error);
    if(observation.error)throw Error(observation.error);
    const slots=await navigation('world.playerWorlds'),slot=slots.slots.find(slot=>slot.kind==='godot');
    if(!observation.cancel){report.initializationCancellation={covered:false,reason:'Initialization completed naturally before a cancellation control was available',slot};write();return slot.worldId;}
    assert.ok(slot?.worldId,'the real initializing slot has its assigned identity');
    report.initializationCancellation={covered:false,before:slots,worldId:slot.worldId};write();
    await action('[data-player-world-cancel]');
    await wait(()=>!document.querySelector('[data-player-world-pending]')&&[...document.querySelectorAll('[data-player-world]')].every(button=>!button.disabled));
    const cancelled=await navigation('world.playerWorlds');assert.equal(cancelled.activeKind,'web');assert.equal(cancelled.activeWorldId,webId);assert.equal(cancelled.slots.find(item=>item.kind==='godot')?.worldId,slot.worldId);
    assert.equal(cancelled.slots.find(item=>item.kind==='godot')?.state,'failed');
    const persisted=await navigation('world.list');const draft=persisted.worlds.find(world=>world.id===slot.worldId);assert.ok(draft,'cancelled source world is retained');assert.equal(draft.creation?.stage,'cancelled');
    report.initializationCancellation.after=cancelled;report.initializationCancellation.draft=draft;
    const resumed=await enter('godot');assert.equal(resumed,slot.worldId);
    report.initializationCancellation.covered=true;write();
    check('real Godot preparation cancels back to Web and the same card retries the retained identity',true,{worldId:resumed,stage:draft.creation.stage});return resumed;
  };
  ...
  return{page,record,rpc,native,action,navigation,panel,initialPaint,enter,prepareCancelRetry,attached,chat,switchCards,quit,stop,visual,webVisualState};
...
try{
 ...
 await active.switchCards();const godotId=await active.prepareCancelRetry(webId);report.godotId=godotId;await active.attached('godot',godotId);check('Godot card prepares and attaches a real normal native world',true);const webSaved=await active.panel('world.read',{id:webId});report.webSavedOnSwitch={buildId:webSaved.world.build.id,snapshot:webSaved.world.snapshot};
