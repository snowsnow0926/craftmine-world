...
async function setup(){if(providerConfig){await verifyProvider();}else{const connection=await invoke('codexConnection',{action:'verify',path:codex});assert.equal(connection.code,'ready');assert.equal(connection.model,'gpt-6-astra');assert.equal(connection.effort,'xhigh');report.connection={code:connection.code,model:connection.model,effort:connection.effort,version:connection.version,accountType:connection.account?.type};await invoke('settingsSet',{worldAgentBackend:'codex-cli',codexCliPath:codex});}
  ...
  if(report.worldId){await until(()=>evaluate(`!!document.querySelector('[data-world-entry-tab-form="worlds"]')`),Boolean);await submit('[data-world-entry-tab-form="worlds"]');await until(()=>evaluate(`!!document.querySelector(${JSON.stringify('[data-world-open="'+report.worldId+'"]')})`),Boolean);await submit('[data-world-open="'+report.worldId+'"]');}
    ...
    const worlds=await until(async()=>{const ui=await evaluate(blankGodotUiState);if(ui.error)throw Error(ui.error);const state=await nav('world.list');const row=state.worlds.find(item=>item.id===state.activeWorldId);if(row&&!existingIds.has(row.id)&&row.state==='failed')throw Error('BLANK_WORLD_INITIALIZATION_FAILED: '+JSON.stringify(row.creation));return state;},state=>state.activeWorldId&&!existingIds.has(state.activeWorldId)&&state.worlds.find(row=>row.id===state.activeWorldId)?.state==='ready');
...
async function createTemplateWorld(input){
  ...
  const existing=new Set(before.worlds.map(row=>row.id));const after=await until(async()=>{const ui=await evaluate(templateCopyReadScript);if(ui.error||ui.entryError)throw Error(ui.error||ui.entryError);const worlds=await nav('world.list'),created=worlds.worlds.filter(row=>!existing.has(row.id));if(created.some(row=>row.state==='failed'))throw Error('TEMPLATE_COPY_CREATION_FAILED: '+JSON.stringify(created));assert(created.length<=1,'TEMPLATE_COPY_MULTIPLE_NEW_WORLDS');return worlds;},worlds=>worlds.activeWorldId&&!existing.has(worlds.activeWorldId)&&worlds.worlds.find(row=>row.id===worlds.activeWorldId)?.state==='ready');
