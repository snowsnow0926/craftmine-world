...
async function connectRenderer(){socket?.close();socket=null;
  let tabs;try{tabs=(await(await fetch('http://127.0.0.1:'+run.port+'/json/list',{signal:AbortSignal.timeout(10000)})).json()).filter(t=>t.type==='page'&&t.url.includes('/out/renderer/index.html')&&!new URL(t.url).searchParams.has('surface'));}catch{throw Error('PAGE_CONNECTION_UNAVAILABLE:owned-renderer-target');}if(tabs.length!==1)throw Error('PAGE_CONNECTION_UNAVAILABLE:owned-renderer-target');
