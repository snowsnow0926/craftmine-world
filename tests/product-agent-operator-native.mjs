...
if(args.includes('--help')){console.log('node tests/product-agent-operator-native.mjs --application-root ABS --runtime-resources ABS (--codex ABS | --provider-config-file ABS) --output-root ABS [--starter promo-city|promo-mainline] [--resume ABS_REPORT] [--packaged-root ABS]\nNo prompts are sent until an explicit inbox command. See docs/PRODUCT_AGENT_OPERATOR_DRIVER.md.');process.exit(0);}
...
const providerConfig=providerConfigFile?parseOperatorProviderConfig(fs.readFileSync(providerConfigFile,'utf8')):null;
...
const sourceTemplate=option('--starter')??previous?.sourceTemplate??'promo-city';assert(['promo-city','promo-mainline',...(previous?.retainedCopy?['retained']:[])].includes(sourceTemplate),'SUPPORTED_OPERATOR_STARTER_REQUIRED');
...
async function configureProvider(){
  if(report.providerConfiguration){await verifyProvider();return;}
