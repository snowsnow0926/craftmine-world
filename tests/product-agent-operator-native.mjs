...
const confirmCreatedWorld=(existingIds,readUiError)=>confirmOperatorCreatedWorld({until,readList:()=>nav('world.list'),readUiError,existingIds,onReadTimeout:recordInitializationReadTimeout});
...
async function createTemplateWorld(input){
  ...
  const existing=new Set(before.worlds.map(row=>row.id));const after=await confirmCreatedWorld(existing,async()=>{const ui=await evaluate(templateCopyReadScript);return ui.error||ui.entryError;});
