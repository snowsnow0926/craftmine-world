...
async function evaluate(expression){const current=socket;return new Promise((resolve,reject)=>{const id=++sequence,timer=setTimeout(()=>{current.removeEventListener('message',listener);reject(Error('PAGE_RPC_TIMEOUT'));},120000),listener=event=>{const value=JSON.parse(event.data);if(value.id!==id)return;clearTimeout(timer);current.removeEventListener('message',listener);value.error||value.result?.exceptionDetails?reject(Error(redact(value.result?.exceptionDetails?.exception?.description??JSON.stringify(value.error)))):resolve(value.result?.result?.value);};current.addEventListener('message',listener);current.send(JSON.stringify({id,method:'Runtime.evaluate',params:{expression,awaitPromise:true,returnByValue:true}}));});}
...
async function inspect(){const events=await evaluate('operatorEvents.splice(0)');if(events.length)fs.appendFileSync(path.join(out,'agent-events.ndjson'),events.map(row=>JSON.stringify(redact(row))).join('\n')+'\n');for(const event of events){if(event.event?.type==='status'&&event.event.status?.backend&&!providerConfig){assert.equal(event.event.status.backend,'codex-cli');assert.equal(event.event.status.modelId,'gpt-6-astra');assert.equal(event.event.status.reasoningEffort,'xhigh');const turn=report.turns.find(row=>row.turnId===event.turnId);if(turn)turn.effectiveModel={backend:event.event.status.backend,model:event.event.status.modelId,effort:event.event.status.reasoningEffort};}}
  ...
  writeJson(path.join(out,'status.json'),snapshot);report.lastStatusAt=snapshot.at;save();return snapshot;
}
