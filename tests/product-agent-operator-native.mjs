...
if(args.includes('--help')){console.log('node tests/product-agent-operator-native.mjs --application-root ABS --runtime-resources ABS --codex ABS --output-root ABS [--starter promo-city|promo-mainline] [--resume ABS_REPORT] [--packaged-root ABS]\nNo prompts are sent until an explicit inbox command. See docs/PRODUCT_AGENT_OPERATOR_DRIVER.md.');process.exit(0);}
...
const sourceTemplate=option('--starter')??previous?.sourceTemplate??'promo-city';assert(['promo-city','promo-mainline'].includes(sourceTemplate),'SUPPORTED_OPERATOR_STARTER_REQUIRED');
...
try{save();console.log(JSON.stringify({out,reportFile,inbox:path.join(out,'inbox'),status:path.join(out,'status.json'),cancelFile}));await start();await setup();if(sourceTemplate==='promo-city'&&(!previous||!previous.ready&&previous.turns.length===0))await composition();report.ready=true;save();console.log('OPERATOR_READY '+out);
