...
import{useAppStore,createCopiedWorldSession,materializeDraftSession}from'./src/stores/app-store';import{WorldCreatePanel}from'./src/components/craftmine/WorldCreatePanel';import{api}from'./src/lib/api';import{writeComposerDraft,readComposerDraft,HOME_DRAFT_KEY}from'./src/lib/composer-draft-cache';import{loadCraftmineLayout,saveCraftmineLayout}from'./src/lib/craftmine-layout';
...
f.createSession=async()=>{api.createSession=async options=>({session:{id:'created-'+(++f.version),title:'New',mode:'agent',permissionMode:'auto',createdAt:1,updatedAt:1,messageCount:0,...options}});return createCopiedWorldSession('new-world',undefined);};
f.pendingAsset=()=>useAppStore.setState({composerPrefill:{worldId:'w1',text:'exact asset request',fileReferences:[],append:true,focus:false}});
f.materialize=()=>materializeDraftSession();f.state=()=>useAppStore.getState();f.draft=()=>readComposerDraft(HOME_DRAFT_KEY);
f.preflight=()=>{f.version++;f.createCalls=0;f.gate();const controller={capabilities:{bases:[{id:'creation-sandbox',delivered:true,starters:[]}],starters:[]},createAttempt:{input:{title:'New',baseId:'creation-sandbox',operationId:'new-operation'}},busy:false,create:async()=>{f.createCalls++;return true;}};app.render(<WorldCreatePanel key={f.version} controller={controller} lang='en' onClose={()=>{}} onBeforeCreate={async()=>{f.preflightStarted=true;await f.hold;return async()=>{};}}/>);};f.changeDraft=()=>writeComposerDraft(HOME_DRAFT_KEY,{text:'等待时新写的内容',fileReferences:[]});f.gate=()=>{f.hold=new Promise(resolve=>f.release=resolve);};f.reuse=()=>{f.version++;app.render(<SourceReusePanel key={f.version} worldId='w1' running={false}/>);};f.reset();`;
...
try{
 ...
 await page.evaluate(()=>fixture.reset());await ready();await page.evaluate(()=>fixture.createSession());check('new world session does not adopt unrelated home draft or files',await page.evaluate(()=>fixture.draft().text==='保留我的愿望'&&fixture.draft().fileReferences.length===1));
 await page.evaluate(()=>{fixture.reset();fixture.pendingAsset();});await ready();await page.evaluate(()=>fixture.materialize());check('home asset reference follows draft materialization and keeps its world',await page.evaluate(()=>fixture.state().composerPrefill.sessionId===fixture.state().activeSessionId&&fixture.state().composerPrefill.worldId==='w1'));
 await page.evaluate(()=>fixture.preflight());await page.waitForSelector('[data-world-create="form"]');await submit('[data-world-create="form"]');await page.waitForFunction(()=>fixture.preflightStarted);await page.evaluate(()=>fixture.reuse());await page.waitForSelector('[data-source-proposal]');await page.evaluate(()=>fixture.release());await page.waitForTimeout(50);check('unmounted creation preflight cannot start a late world create',await page.evaluate(()=>fixture.createCalls===0));
