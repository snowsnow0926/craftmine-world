...
import {inspectAdoptionSource,adoptionEnvironment,validateAdoptionCall} from './helpers/promo-adoption-contract.mjs';
...
const record={format:'craftmine.promo-adoption/1',originalReport:file,worldId:selection.worldId,candidateId:selection.candidateId,checkedBuildId:selection.buildId,budgetBefore:selection.budget,budgetAfter:null,modelRequestsAdded:null,sourceEdits:0,checks:[],launches:[],visual:'UNVERIFIED'};
...
const rpc=(method,fields={})=>new Promise((resolve,reject)=>{validateAdoptionCall(method,fields,selection);const id=randomUUID(),timer=setTimeout(()=>{pending.delete(id);reject(Error('TIMEOUT '+method));},120000);pending.set(id,{resolve,reject,timer});child.send({type:'craftmine-headless',id,method,...fields});});
...
try{
  ...
  await stop();assertProofs(proofs);check('original report and request ledger remain byte-identical',true);record.budgetAfter={...selection.budget};record.modelRequestsAdded=0;record.ok=true;
