...
test('recorded pinned-engine import matches this inventory and preserves the doorway opening',()=>{
 ...
 for(const scene of validation.scenes){const item=manifest.items.find(item=>item.id===scene.id);assert.equal(scene.triangles,item.geometry.triangles);assert.ok(scene.meshInstances>0);if(item.source==='castle')assert.ok(scene.texturedSurfaces>0);}
});
