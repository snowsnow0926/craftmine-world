...
finally{await context?.close();await new Promise(resolve=>server.close(resolve));report.elapsedMs=performance.now()-benchmarkStarted;report.processElapsedMs=performance.now();report.sourceHashes=Object.fromEntries(['app/game.js','world-workshop-3d/src/voxel-runtime.js','vendor/pi-desktop/apps/desktop/electron/main/craftmine-telemetry.ts'].map(file=>[file,createHash('sha256').update(fs.readFileSync(path.join(root,file))).digest('hex')]));save();console.log('Performance report: '+path.join(out,'report.json'));}
