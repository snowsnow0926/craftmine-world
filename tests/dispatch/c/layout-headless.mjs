...
fs.mkdirSync(path.join(root,'test-results'),{recursive:true});const out=fs.mkdtempSync(path.join(root,'test-results/dispatch-c-layout-'));
...
const entry=path.join(out,'fixture.jsx');fs.writeFileSync(entry,`import React from 'react';import {createRoot} from 'react-dom/client';import {CraftmineLayoutControls} from ${JSON.stringify(component)};import * as layout from ${JSON.stringify(layout)};window.layout=layout;createRoot(document.getElementById('root')).render(<CraftmineLayoutControls/>);`);
...
fs.writeFileSync(path.join(out,'index.html'),`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>:root{--ds-text-secondary:#b5beb8;--ds-text-primary:#eff7f0;--ds-bg-active:#284235;--radius-xs:6px;--text-2xs:13px}body{background:#191919;color:#eee;font:14px system-ui;padding:30px}button{cursor:pointer}h1{font-size:16px}${fs.readFileSync(path.join(desktop,'src/styles/craftmine.css'),'utf8')}</style></head><body><h1>工作台布局 · React 组件夹具</h1><div id="root"></div><script src="fixture.js"></script></body></html>`);
...
try{
  context=await playwright().chromium.launchPersistentContext(path.join(out,'profile'),{...browserOptions(),viewport:{width:640,height:300}});await context.addInitScript(()=>{window.__audit={lock:0,focus:0};Element.prototype.requestPointerLock=()=>{__audit.lock++;throw Error('Input lock disabled');};window.focus=()=>{__audit.focus++;};HTMLElement.prototype.focus=function(){__audit.focus++;};});
