...
function domain(calls){return async(method,params)=>{calls.push({method,params});if(method==='backup.export')return{id:params.operationId,status:'completed',archive,manifest:{hash:h}};if(method==='backup.inspect')return{valid:params.archive.hash===h,hash:h,counts:{worlds:1}};if(method==='backup.status')return{currentHash:old};if(method==='backup.restore'){if(params.expectedCurrentHash!==old)throw Object.assign(Error('STALE_WORLD'),{code:'STALE_WORLD'});return{id:params.operationId,status:'completed',currentHash:h,modelReplay:false};}if(method==='backup.cancel')return{status:'cancelled'};throw Error(method);};}
test('export uses a native choice and restores only a content-bound opaque grant',async()=>{
  const f=await fixture(),calls=[],service=createCraftmineBackupService({domainCall:domain(calls),pickFile:async()=>f.file});
  ...
  assert.equal(calls.filter(c=>c.method==='backup.restore').length,1);
});
test('renderer paths and raw archives cannot become filesystem authority',async()=>{
  const service=createCraftmineBackupService({domainCall:async()=>{throw Error('must not call');},pickFile:async()=>{throw Error('must not choose');}});
...
test('file replacement after preview and stale world hash prevent restore',async()=>{
  ...
  const service=createCraftmineBackupService({domainCall:domain(calls),pickFile:async()=>f.file}),inspected=await service.request('backup.inspect');
  ...
  assert.equal(calls.filter(c=>c.method==='backup.restore').length,0);
...
test('lost restore receipt can be explicitly retried using the same operation id',async()=>{
  ...
  const call=domain(calls),service=createCraftmineBackupService({pickFile:async()=>f.file,domainCall:async(method,params)=>{const result=await call(method,params);if(method==='backup.restore'&&first){first=false;throw Object.assign(Error('lost'),{code:'TRANSPORT_LOST'});}return result;}});
  ...
  assert.equal(calls.filter(c=>c.method==='backup.restore').every(c=>c.params.operationId==='lost-fixture'),true);
});
test('cancelled picker cannot start export and expired grants cannot restore',async()=>{
  ...
  const service=createCraftmineBackupService({domainCall:domain(calls),pickFile:()=>picked});const pending=service.request('backup.export',{operationId:'cancel-fixture'});await service.request('backup.cancel',{operationId:'cancel-fixture'});choose(f.file);assert.equal((await pending).status,'cancelled');assert.equal(calls.some(c=>c.method==='backup.export'),false);
  await fs.writeFile(f.file,JSON.stringify(archive));let now=0;const expiring=createCraftmineBackupService({domainCall:domain(calls),pickFile:async()=>f.file,now:()=>now});const grant=await expiring.request('backup.inspect');now=700000;
...
test('corrupt and oversized selected files fail without leaking their paths',async()=>{
  const f=await fixture(),calls=[],service=createCraftmineBackupService({domainCall:domain(calls),pickFile:async()=>f.file});await fs.writeFile(f.file,'broken JSON');await assert.rejects(service.request('backup.inspect'),/BACKUP_INVALID_JSON/);
  const handle=await fs.open(f.file,'w');await handle.truncate(32*1024*1024+1);await handle.close();await assert.rejects(service.request('backup.inspect'),/TOO_LARGE/);
