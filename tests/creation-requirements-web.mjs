...
try{
 ...
 const entities=[entity('tree-a','tree',[4,0,0],[2,2,2]),entity('door-a','door',[-4,0,0]),entity('marker-a','marker',[0,0,0]),entity('marker-b','marker',[0,0,-4])];
 for(const variant of ['correct','unconditional']){
  ...
  check(variant+' real engine answers entities',actual.length===4);
  ...
  if(variant==='correct'){const trace=await verifyCreationDoorSequence(runtime,required,defaults,p=>p);report.trace=trace;check('real sequence requires ordered interactions',creationDoorTraceMatches(required,trace));}
