// Runner for the C failure/cancel/recovery acceptance. Same private offscreen
// Electron environment as `full-chain.mjs`; no input, focus or window activation.
...
const out = fs.mkdtempSync(path.join(root, 'test-results', 'godot-remaining-c-failures-'));
...
fs.writeFileSync(path.join(appDir, 'package.json'), JSON.stringify({ name:'c-failures', main:'main/index.cjs' }));
for (const [entry, destination] of [
  ['tests/godot-remaining/C/failures-electron.mjs', 'main/index.cjs'],
