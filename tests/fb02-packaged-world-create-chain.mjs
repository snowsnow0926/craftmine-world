...
try{
  ...
  if(expectFixed){
    ...
    if(process.argv.includes('--normal-create')){
      ...
      report.cancelledRuntime=await readRuntime(created.id);assert.equal(report.cancelledRuntime.buildId,report.normalCreatedRuntime.buildId,'cancel does not replace original formal build');await click('[data-mode-entry] > .craftmine-mode-back');await delay(500);assert.equal((await snapshot()).session,originalSession,'cancel does not bind the draft to the original conversation');
      ...
      await appPage.waitForSelector('[data-world-continue="'+retainedId+'"]');await click('[data-world-continue="'+retainedId+'"]');
      await appPage.waitForFunction(id=>document.querySelector('[data-world-id="'+id+'"][data-world-playable="true"]')&&document.querySelector('.craftmine-mode-enter-world')?.dataset.activeWorld===id,retainedId,{timeout:120000});
