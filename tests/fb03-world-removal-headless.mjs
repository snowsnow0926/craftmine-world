...
report.cancellationRoute={cancel:await host('godotWorld.initCancel',{worldId}),state:await host('godotWorld.initStatus',{worldId})};
assert.equal(report.cancellationRoute.cancel.status,'cancelled');assert.equal(report.cancellationRoute.state.cancelled,true);
report.cancellationRoute.clear=await host('godotWorld.initCancelClear',{worldId});assert.equal(report.cancellationRoute.clear.cleared,true);
assert.equal((await host('godotWorld.initStatus',{worldId})).status,'blocked');
