...
| `backups::portable::durability_tests` | a real separate process is ended abruptly inside the restore at `after-claim`, `after-stage`, `after-content`, `after-git` and `after-commit`; recovery undoes exactly the journaled work or promotes the commit from the receipt written before it; a lookalike staging directory is refused and survives; a cancel request converges |
