...
export function extractOperatorUsage({reports=[],events=[],sessions=[],generatedAt=new Date().toISOString()}){
  ...
  function group(sessionId,turnId){const id=key(sessionId,turnId);if(!groups.has(id))groups.set(id,{sessionId,turnId,reports:[],metrics:[],snapshots:[],terminalMessages:[],terminalEvents:[],startEvents:[],discoveredUsers:[],models:[],toolCalls:new Set(),warnings:[],rejectedUsageReports:[],coverageReports:[]});return groups.get(id);}
  ...
  function coverage(g,value,source,kind){
    if(value===undefined||value===null)return;
    const valid=value.status==='incomplete'&&value.reason==='native-maintenance-usage-unreported'&&integer(value.maintenanceTurns)&&(value.maintenanceElapsedMs===null||integer(value.maintenanceElapsedMs));
    if(!valid)g.warnings.push('INVALID_CODEX_USAGE_COVERAGE');
    const partial=usage(g,value.reportedCreationUsage,{...source,field:(source.field??'event.status.codexUsageCoverage')+'.reportedCreationUsage'},'reported-creation-partial');
    g.coverageReports.push({source,kind,rawReported:structuredClone(value),valid,reportedCreationUsage:partial});
  }
  ...
  for(const entry of events){
    ...
    if(event.type==='status'){
      ...
      coverage(g,status.codexUsageCoverage,{...source,field:'event.status.codexUsageCoverage'},'status-event');
    ...
    if(event.type==='message_end'&&event.message?.codexUsage?.scope==='current-turn')coverage(g,event.message.codexUsage.coverage,{...source,field:'event.message.codexUsage.coverage'},'terminal-message-event');
  ...
  for(const entry of sessions){
    ...
    for(const [index,message]of session.messages.entries()){
      ...
      if(message.role!=='assistant'||message.codexUsage?.scope!=='current-turn'||(!message.usage&&!message.codexUsage.coverage))continue;
      ...
      coverage(g,message.codexUsage.coverage,{...source,field:source.field.replace(/\.usage$/,'.codexUsage.coverage')},'persisted-terminal-message');
  ...
  for(const g of groups.values()){
    ...
    const hasCoverage=g.coverageReports.length>0;
    // Coverage observations are snapshots too. Prefer terminal carriers, retain
    // every source, and never add maintenance counts/elapsed across duplicates.
    const finalCoverage=g.coverageReports.filter(row=>row.kind!=='status-event');
    const coverageCandidates=finalCoverage.length?finalCoverage:g.coverageReports.slice(-1);
    const metadata=new Map(coverageCandidates.filter(row=>row.valid).map(row=>[JSON.stringify({status:row.rawReported.status,reason:row.rawReported.reason,maintenanceTurns:row.rawReported.maintenanceTurns,maintenanceElapsedMs:row.rawReported.maintenanceElapsedMs}),row]));
    const coverageConflict=metadata.size>1||g.coverageReports.some(row=>!row.valid);
    if(metadata.size>1)g.warnings.push('CONFLICTING_CODEX_USAGE_COVERAGE');
    const selectedCoverage=!coverageConflict&&metadata.size===1?[...metadata.values()][0]:null;
    const partials=new Map(coverageCandidates.filter(row=>row.valid&&row.reportedCreationUsage).map(row=>[JSON.stringify(row.reportedCreationUsage),row]));
    const invalidPartial=coverageCandidates.some(row=>row.rawReported.reportedCreationUsage!=null&&!row.reportedCreationUsage);
    if(partials.size>1)g.warnings.push('CONFLICTING_REPORTED_CREATION_PARTIALS');
    const selectedPartial=!coverageConflict&&!invalidPartial&&partials.size===1?[...partials.values()][0]:null;
    const usageCoverage=hasCoverage?{status:'incomplete',reason:selectedCoverage?.rawReported.reason??'unresolved-coverage-metadata',maintenanceTurns:selectedCoverage?.rawReported.maintenanceTurns??null,maintenanceElapsedMs:selectedCoverage?.rawReported.maintenanceElapsedMs??null,source:selectedCoverage?.source??null,provisional:!isTerminal||!finalCoverage.length}:null;
    ...
    const fullCalls=!hasCoverage&&raw?.coverage==='complete'&&integer(raw.calls?.observed)&&raw.calls.observed>0&&raw.calls.pending===0;
    ...
    rows.push({sessionId:g.sessionId,turnId:g.turnId,messageId:known?.messageId??discovered?.id??null,promptExcerpt:typeof known?.text==='string'?known.text.slice(0,240):discovered?.text?.slice(0,240)??null,discovery:g.reports.length?'operator-report':'session-event-turn',status,terminal:isTerminal,model,
      elapsed:timing,finalUsage:isTerminal&&chosen&&!hasCoverage&&!rejectedTerminal&&!g.warnings.includes('TERMINAL_SOURCES_DISAGREE')?{...chosen.usage,source:chosen.source,kind:chosen.kind,scope:'one-turn'}:null,
      usageAvailability:hasCoverage?'incomplete-native-maintenance-usage':rejectedTerminal?'rejected-terminal-usage':chosen?isTerminal?'terminal-reported':'terminal-message-awaiting-turn-close':snapshot?'last-observed-only':g.rejectedUsageReports.length?'invalid-reported-usage':'unknown',
      usageCoverage,coverageReports:g.coverageReports,reportedCreationUsage:selectedPartial?{...selectedPartial.reportedCreationUsage,scope:'reported-creation-only-excludes-native-maintenance',source:{...selectedPartial.source,field:selectedPartial.source.field+'.reportedCreationUsage'},provisional:!isTerminal||!finalCoverage.length}:null,
      ...
      modelCalls:{value:fullCalls?raw.calls.observed:null,coverage:hasCoverage?'incomplete-native-maintenance':raw?.coverage??'unknown',scope:raw?.scope??null,rawObserved:raw?.calls?.observed??null,rawReported:raw?.calls?.reported??null,rawPending:raw?.calls?.pending??null,source:metric?.source??null,reason:fullCalls?'complete-host-model-call-ledger':hasCoverage?'native-maintenance-call-coverage-incomplete':'no-complete-physical-model-call-ledger'},
  ...
  return {format:'craftmine.operator-usage-evidence/1',generatedAt,turns:rows,finalAggregate:aggregate,
    aggregateAvailability:final?'all-observed-turns-terminal-and-reported':rows.some(row=>!row.terminal)?'ongoing-turns-no-final-total':rows.some(row=>row.usageCoverage)?'native-maintenance-usage-unreported-no-final-total':'missing-or-conflicting-final-usage',
...
export function operatorUsageMarkdown(report){
  ...
  const incomplete=report.turns.filter(row=>row.usageCoverage);
  if(incomplete.length){
    lines.push('','原生维护消耗未报告；下列数值只是已报告的创作部分，不是该轮完整用量，也不能与维护次数相加。','','| 轮次 | 已报告创作 token | 已完成维护次数 | 维护观测耗时（秒） |','|---|---:|---:|---:|');
    for(const row of incomplete)lines.push(`| ${row.turnId} | ${row.reportedCreationUsage?.provisional?'—（运行中）':display(row.reportedCreationUsage?.totalTokens)} | ${display(row.usageCoverage.maintenanceTurns)} | ${integer(row.usageCoverage.maintenanceElapsedMs)?(row.usageCoverage.maintenanceElapsedMs/1000).toFixed(2):'—'} |`);
  }
