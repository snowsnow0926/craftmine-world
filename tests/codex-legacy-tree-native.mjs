...
const worldRead=(channel,payload={})=>worldEvaluate(`pluginBridge.invoke(${JSON.stringify(channel)},${JSON.stringify(payload)})`);
...
async function start(label){
  ...
  await until(async()=>{try{return await nav('world.createOptions');}catch(error){if(/WORLD_PLUGIN_NOT_LOADED|not loaded|not ready/i.test(String(error)))return null;throw error;}},value=>value?.bases?.some(base=>base.id===LEGACY_TREE_BASE&&base.delivered));
...
try{
  ...
  mark('selected-codex-connection-configured');await stop();await start('creation');
  assert.equal((await invoke('settingsGet')).worldAgentBackend,'codex-cli');
  await chooser('create');await until(()=>evaluate(`!!document.querySelector('[data-world-base-option="${LEGACY_TREE_BASE}"] input')`),Boolean);
  ...
  report.worldId=(await nav('world.list')).activeWorldId;await bindWorld();report.before=await worldRead('world.read',{id:report.worldId});assert.equal(report.before.runtimeKind,'legacy');assert.equal(report.before.world.build.scene.objects.length,0);mark('ordinary-blank-web-world-created');
  ...
  const metrics=await until(async()=>{await drainEvents();const value=await invoke('sessionTurnMetrics',{sessionId:report.sessionId,messageId:message.id});report.metrics=value;save();return value;},value=>['completed','error','aborted'].includes(value.status));
