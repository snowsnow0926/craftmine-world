...
const calls=[],statuses=new Map();let refreshes=0,dropNext=false,render;globalThis.__craftmineWorldBridge={invoke:async(_,channel,payload)=>{calls.push({channel,payload});if(channel==='godot.creationPreview')return{status:payload.action==='cancel'?'cancelled':'visible',valid:true};if(channel==='godot.creationEditHistory')return{latestUndoOperationId:'previous-edit'};if(channel==='godot.creationEdit'){const status={...payload,phase:'checking'};statuses.set(payload.operationId,status);if(dropNext){dropNext=false;throw Error("transport reply lost");}return status;}if(channel==='godot.creationEditStatus')return{...statuses.get(payload.operationId),phase:'applied',receipt:{operationId:payload.operationId,undoSupported:true}};return{};},onChanged:()=>()=>{}};
const controller={sessionId:'session-a',capture:{captureId:'capture-a',worldId:'world-a',target:{entityId:'tree-a',entityName:'树',entityPosition:[4,0,5],rotationY:15,scale:[1,1,1],color:'#84a866'}},refresh:async()=>{refreshes++;}};
...
try{
 ...
 check('位置来自对象原点而非射线命中点',await page.evaluate(()=>document.querySelector('[aria-label="位置 X"]').value==='4'&&document.querySelector('[aria-label="朝向角度"]').value==='15'));
 await page.evaluate(()=>fixture.action('预览摆放'));await page.waitForFunction(()=>fixture.calls.some(c=>c.channel==='godot.creationPreview'));
 await page.evaluate(()=>{fixture.change('位置 X','6');fixture.change('朝向角度','45');});await page.waitForFunction(()=>fixture.calls.some(c=>c.channel==='godot.creationPreview'&&c.payload.position?.[0]===6&&c.payload.rotationY===45));
 check('预览更新不创建任务或源码写入',await page.evaluate(()=>!fixture.calls.some(c=>c.channel==='godot.creationEdit')));
 await page.evaluate(()=>fixture.action('取消'));await page.waitForFunction(()=>!document.querySelector('fieldset'));await page.waitForFunction(()=>fixture.calls.some(c=>c.channel==='godot.creationPreview'&&c.payload.action==='cancel'));
 check('关闭编辑发送同一预览的取消操作',await page.evaluate(()=>{const calls=fixture.calls.filter(c=>c.channel==='godot.creationPreview');return calls.at(-1).payload.previewId===calls[0].payload.previewId;}));
