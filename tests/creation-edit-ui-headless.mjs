...
globalThis.fixture={calls,world:value=>{controller.capture={...controller.capture,worldId:value,captureId:"capture-"+value};render();},drop:()=>{dropNext=true;},getRefreshes:()=>refreshes,action:text=>{const element=[...document.querySelectorAll('button')].find(el=>el.textContent===text);if(!element||element.disabled)throw Error('Action unavailable '+text);element[Object.keys(element).find(key=>key.startsWith('__reactProps'))].onClick();},change:(label,value)=>{const element=document.querySelector('[aria-label="'+label+'"]');element[Object.keys(element).find(key=>key.startsWith('__reactProps'))].onChange({target:{value}});}};
