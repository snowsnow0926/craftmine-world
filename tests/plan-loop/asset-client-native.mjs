...
try {
 ...
 await start();await step('actual Main navigation opens the local asset library',openAssets);let beforeWorlds=await nav('world.list');report.beforeWorlds=structuredClone(beforeWorlds);
 const beforeWorldSnapshot=structuredClone((await until(()=>rpc('worldState'),v=>v.loaded&&v.id===ownerWorldId&&v.snapshot,'actual initial world snapshot')).snapshot);report.beforeWorldSnapshot=beforeWorldSnapshot;
 async function assertWorldUnchanged(){assert.deepEqual(await nav('world.list'),beforeWorlds);const actual=await rpc('worldState');assert.equal(actual.id,ownerWorldId);assert.deepEqual(actual.snapshot,beforeWorldSnapshot);}
 ...
 await step('tag edit is acknowledged without another asset changing',async()=>{await view('saveTags',{assetId:assetIds[0],tags:['建筑','常用']});await until(()=>view('read'),v=>v.selected?.tags==='常用, 建筑'&&!v.selected.pending,'tag receipt');await view('filter',{favoritesOnly:false});await until(()=>view('read'),v=>v.cards?.length===2,'all cards');await view('select',{assetId:assetIds[1],version:1});const value=await until(()=>view('read'),v=>v.selected?.assetId===assetIds[1],'second asset');assert.equal(value.selected.favorite,false);assert.equal(value.selected.tags,'');await assertWorldUnchanged();return value;});
 await step('tags can be cleared and old-owner operations are refused',async()=>{
  ...
  await assertWorldUnchanged();return{cleared:true,staleOwnerRejected:true};
 });
 ...
 await step('real isolated decoder produces a visible PNG preview',async()=>{await view('preview',{assetId:importedId,version:1});const state=await until(()=>view('read'),v=>v.preview?.tone==='success'&&v.preview.thumbnail&&!v.error,'decoded PNG thumbnail');await assertWorldUnchanged();return state;});
 ...
 await withCore(async c=>{
  assert.deepEqual(await immutable(c),report.beforeAssets);report.afterFirstAssets=await immutable(c);
  // The retained legacy view saves its initialized live progress on normal quit.
  // Verify the complete saved snapshot, then bind restart to that actual record.
  const saved=await c.call('world.read',{id:ownerWorldId});assert.deepEqual(saved.world.snapshot,beforeWorldSnapshot);
  const row=beforeWorlds.worlds.find(w=>w.id===ownerWorldId);assert.equal(saved.id,row.id);assert.equal(saved.title,row.title);assert.equal(saved.runtimeKind,row.runtimeKind);
  report.firstSavedWorld=structuredClone(saved);beforeWorlds=structuredClone(beforeWorlds);
  const after=beforeWorlds.worlds.find(w=>w.id===ownerWorldId);after.revision=saved.revision;after.updatedAt=saved.updatedAt;report.restartWorldListBaseline=structuredClone(beforeWorlds);
 });
 await start();await step('reopened actual library retains tags and favorite state',async()=>{await openAssets();await view('select',{assetId:assetIds[0],version:1});const value=await until(()=>view('read'),v=>v.selected?.assetId===assetIds[0],'reopened detail');assert.equal(value.selected.tags,'常用, 建筑');assert.equal(value.selected.favorite,false);await assertWorldUnchanged();return value;});
