...
try{
 ...
 await step('real sidebar guide is optional, keeps game chrome fixed and uses existing navigation',async()=>{const r=await rpc('worldCreationGuide',{},120000);assert.equal(r.worldId,world.id);assert.equal(r.steps,5);for(const key of ['collapsed','expanded','insideSidebar','headerUnchanged','libraryVisible','shareVisible','progressConsentRequired','dismissed','bookmarkRetained','historyVisible','worldUnchanged','oldGuideAbsent'])assert.equal(r[key],true,key);return r;});
