...
function fixture(){const f={world:'alpha',blocked:false,throwCode:null,calls:[],applies:0,status:'queued',receipts:new Map()};
 f.native=createCraftmineTargetFeedbackPanel({selection:async()=>f.world,blocked:()=>f.blocked,domain:async(method,args)=>{
  ...
  if(method==='targetFeedback.describe')return{worldId:'alpha',buildId:'build-a',targets:[{targetId:'target_a',label:'训练靶 <script>',configuration:targetFeedbackConfiguration(),binding,values:{hitFlashMilliseconds:120},hostPath:'private'}],unsupported:[],hostContext:'private'};
...
test('panel translates bounded source binding and projects no private response fields',async()=>{
 const f=fixture(),result=await f.native('targetFeedback.describe',{worldId:'alpha'});assert.deepEqual(result.targets[0].sourceBinding,binding);assert.equal(JSON.stringify(result).includes('private'),false);
