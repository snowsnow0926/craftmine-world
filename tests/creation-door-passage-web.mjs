...
try{
 ...
 for(const variant of ['correct','ghost-closed','retained-open']){
  const project=path.join(out,variant),exportRoot=path.join(out,variant+'-web');fs.mkdirSync(exportRoot);materializeBase({baseId:'creation-sandbox',worldId:'passage-world',out:project});
  ...
  const worldFile=path.join(project,'scripts/creation_world.gd');if(variant!=='correct'){const source=fs.readFileSync(worldFile,'utf8'),needle='shape.set_deferred("disabled", doors.get(id, false))';assert.equal(source.split(needle).length,2);fs.writeFileSync(worldFile,source.replace(needle,'shape.set_deferred("disabled", '+(variant==='ghost-closed'?'true':'false')+')'));}
  ...
  const entry={variant,requirements,instanceId:runtime.instanceId};report.cases.push(entry);
  try{
  ...
  }catch(error){entry.error=String(error.message);entry.passage=error.passage;entry.status='failed';if(variant==='correct')throw error;assert.equal(entry.error,variant==='ghost-closed'?'CREATION_PASSAGE_CLOSED_NOT_BLOCKING':'CREATION_PASSAGE_OPENED_NOT_TRAVERSABLE');assert.ok(entry.passage);}
