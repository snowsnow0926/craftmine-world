...
assert.ok(args[0]&&args[1]&&args[2],'Usage: node tests/fb03-ordinary-player-loop.mjs APP_DIR SOURCE_PROFILE WORLD_ID [--flow retained|dialogue] --config ABS_JSON --plan|--live');
const flow=option('--flow','retained');assert.ok(['retained','dialogue'].includes(flow));
const sourceProfile=path.resolve(args[1]),retainedWorldId=args[2];assert.match(retainedWorldId,/^[a-zA-Z0-9._-]{1,128}$/);
const pack=path.resolve(args[0]),configFile=path.resolve(option('--config',path.join(repo,'test-results/fb03-player-config-snapshot.json')));
const secretsFile=path.resolve(option('--secrets','D:/Craftmine World/.craftmine/secrets.json'));
const config=JSON.parse(fs.readFileSync(configFile,'utf8'));
const textFile=option('--text',null),text=textFile?fs.readFileSync(path.resolve(textFile),'utf8').trim():flow==='dialogue'?'创造一片能走进去探索的森林':'给我生成一个AK47';
...
assert.equal(config.mode,'agent');assert.equal(config.sourceGlobalPermissionMode??null,null,'This default-entry scenario requires an unset original global permission preference');assert.equal(config.modelBinding.id,config.modelId);
...
if(args.includes('--plan')||!args.includes('--live')){
  console.log(JSON.stringify({mode:'prepare-only',package:pack,configFile,secretsFile,modelId:config.modelId,thinkingLevel:config.thinkingLevel,contextWindow:config.contextWindow,maxTokens:config.maxTokens,
    inputPath:flow==='dialogue'?'preparation text input + explicit Send when ready + ordinary host prompt':'retained world entry + ordinary Composer input and send callback',text,modelRequests:0,roundDeadline:null,modelCallLimit:null,automaticRepairLimit:null,noSourceEdits:true},null,2));process.exit(0);
}
const directory=fs.mkdtempSync(path.join(repo,'test-results/desktop-native-fb03-ordinary-')),profile=path.join(directory,'profile'),legacy=path.join(directory,'legacy'),token=randomUUID();
fs.mkdirSync(profile);fs.mkdirSync(legacy);
if(flow==='retained'){
const from=path.join(sourceProfile,'plugins/data/craftmine.world'),to=path.join(profile,'plugins/data/craftmine.world');fs.mkdirSync(to,{recursive:true});
...
const report={format:'craftmine.fb03-ordinary-player/1',sourceProfile,flow,package:pack,development,mainBundleSha256:createHash('sha256').update(main).digest('hex'),directory,profile,config,text,startedAt:new Date().toISOString(),status:'PREPARING',
...
let sessionId,worldId,active,queuedByPreparation=false;
...
try{
  ...
  const created=await active.api('providersCreate',{name:'FB03 isolated original player model',vendorKey:config.vendorKey,protocol:config.protocol,type:config.protocol,baseUrl:config.baseUrl,authKind:'api_key_and_base_url',secretValue:secret,apiStyle:config.apiStyle,defaultModelId:config.modelId,models:[config.modelBinding]});
  const providerId=created.provider.id,settings=await active.api('settingsGet');
  const nextSettings={...settings,defaultProviderId:providerId,defaultModelId:config.modelId,defaultMode:config.mode,language:'zh-CN'};
  delete nextSettings.defaultPermissionMode;
  await active.api('settingsSet',nextSettings);
  ...
  if(flow==='dialogue'){
    ...
    await until(()=>active.appPage.evaluate(()=>!!document.querySelector('#dialogue-preparation-draft')&&!document.querySelector('#dialogue-preparation-draft').readOnly),Boolean);
    await active.appPage.evaluate(text=>{const input=document.querySelector('#dialogue-preparation-draft');input[Object.keys(input).find(k=>k.startsWith('__reactProps'))].onChange({target:{value:text}});},text);
    await until(()=>active.appPage.evaluate(text=>document.querySelector('#dialogue-preparation-draft')?.value===text,text),Boolean);
    report.preparationQueuedAt=new Date().toISOString();report.status='DIALOGUE_PREPARING';save();
    await active.appPage.evaluate(()=>{const button=[...document.querySelectorAll('.dialogue-preparation-composer button')].find(el=>el.textContent==='准备好后发送');if(!button||button.disabled)throw Error('QUEUE_BUTTON_UNAVAILABLE');button[Object.keys(button).find(k=>k.startsWith('__reactProps'))].onClick();});
    queuedByPreparation=true;
    ...
    sessionId=dialogue.sessionId;worldId=(await active.panel('world.list')).activeWorldId;
  }else{
    await until(()=>active.appPage.evaluate(()=>!!document.querySelector('.craftmine-mode-enter-world')&&!document.querySelector('.craftmine-mode-enter-world').disabled),Boolean);
    await active.react('.craftmine-mode-enter-world');worldId=retainedWorldId;
  ...
  check('test setup never silently sets the global permission to Auto',report.settings.defaultPermissionMode!=='auto');save();
...
}catch(error){report.error=redact(String(error.stack??error));if(signal.aborted)report.status='CANCELLED';else report.status='FAILED';process.exitCode=1;console.log(report.error);}
finally{
