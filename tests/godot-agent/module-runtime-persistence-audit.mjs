...
fs.mkdirSync(path.join(root,'test-results'),{recursive:true});const out=fs.mkdtempSync(path.join(root,'test-results/module-persistence-audit-')),project=path.join(out,'project'),tasksRoot=path.join(out,'tasks');fs.mkdirSync(project);fs.mkdirSync(tasksRoot);
...
fs.copyFileSync(path.join(root,'tests/fixtures/module-runtime-persistence.gd'),path.join(project,'module_persistence.gd'));
const projectFile=path.join(project,'project.godot');fs.writeFileSync(projectFile,fs.readFileSync(projectFile,'utf8').replace('[autoload]','[autoload]\nModulePersistence="*res://module_persistence.gd"'));
...
const id='gmp-'+Date.now(),request={schemaVersion:1,requestId:id,taskId:id,operation:'exportWeb',projectRoot:project,tasksRoot,engineRoot,sourceBinding:{worldId:formal.worldId,buildId:id,sourceRevision:1,sourceDigest},inputHash:hash(id)};
const report={format:'craftmine.module-persistence-audit/1',out,sourceReport,formalSourcePin:pin,originalFiles:original,request,originalPickerSha256:hash(fs.readFileSync(path.join(project,'craftmine_shared/scene_mesh_picker.gd')))};
report.fixtureSha256=hash(fs.readFileSync(path.join(project,'module_persistence.gd')));
report.moduleScriptSha256=hash(fs.readFileSync(path.join(project,'addons/kenney-city-building/module.gd')));
assert.equal(report.moduleScriptSha256,'e2341b41e564ddd2b573039aa82f9dbdc9d4637ad4c7e004d527216f3bece4b5');
...
try{
 ...
 report.bridge=await page.evaluate(async scope=>{const call=op=>new Promise(resolve=>{__pending.set(op,resolve);__receiver(JSON.stringify({id:op,op,...scope,args:{}}));});return {load:await call('load'),resume:await call('resume')};},{worldId:formal.worldId,buildId:id,instanceId:'module-persistence-forensic'});
 await page.waitForFunction(()=>globalThis.__MODULE_PERSISTENCE_AUDIT!==undefined,{},{timeout:60000});report.result=await page.evaluate(()=>__MODULE_PERSISTENCE_AUDIT);report.guard=await page.evaluate(()=>__guard);
 await page.screenshot({path:path.join(out,'module-reconstruction.png')});save();assert.equal(report.result.passed,true,JSON.stringify(report.result));assert.deepEqual(report.guard,{pointerLock:0,focus:0});assert.deepEqual(report.pageErrors,[]);assert.deepEqual(report.console.filter(item=>item.type==='error'||/SCRIPT ERROR|Parse Error|ERROR:/.test(item.text)),[]);
