...
app.whenReady().then(async()=>{
 ...
 const before=await host.snapshot();report.paused=await host.request('engine-performance',{nonce:'a'.repeat(64)});
 ...
 for(const args of [{},{nonce:'short'},{nonce:'a'.repeat(64),monitor:'TIME_PROCESS'}])await assert.rejects(host.request('engine-performance',args),/ENGINE_PERFORMANCE_NONCE/);
 assert.deepEqual(await host.snapshot(),before);report.snapshotUnchanged=true;
 await host.resume();await new Promise(r=>setTimeout(r,1600));report.active=await host.request('engine-performance',{nonce:'b'.repeat(64)});
 ...
 assert.equal(report.active.sample.sequence,report.paused.sample.sequence+1);
