...
test('world templates preserve declared paired GLB import settings byte-for-byte',async()=>{
...
test('image and audio originals remain supported while their import sidecars fail explicitly',async()=>{
 for(const extension of ['svg','png','jpg','jpeg','webp','ogg','wav']){
  const asset='assets/source.'+extension,body=Buffer.from('opaque original fixture');
  const result=await validateArchive(archive({[asset]:body}));assert.deepEqual(result.files.get('source/'+asset),body);
  await assert.rejects(validateArchive(archive({[asset]:body,[asset+'.import']:Buffer.from('unsupported policy')})),/WORLD_TEMPLATE_SOURCE_PATH_INVALID/);
 }
});
