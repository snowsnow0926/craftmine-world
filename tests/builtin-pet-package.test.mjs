...
test('whole library is deterministic and appends one playable module without reclassifying old resources',()=>{
 ...
 assert.equal(preserved.filter(entry=>entry.kind==='scene').length,1);
 ...
 assert.equal(completePins.length,22);assert.deepEqual(preserved.map(e=>e.assetId).sort(),completePins.map(e=>e.assetId).sort());
 for(const pin of completePins){const entry=a.entries.find(e=>e.assetId===pin.assetId);assert.equal(entry.version,pin.version);assert.equal(entry.file,pin.file);assert.equal(sha(fs.readFileSync(path.join(output,pin.file))),pin.sha256);}
