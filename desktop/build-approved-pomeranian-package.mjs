...
export function buildApprovedPomeranianPackage({repository,root=path.join(repository,'desktop/godot/components/approved-pomeranian')}){
  ...
  const bytes=packStaticPackage({root:{id:APPROVED_POMERANIAN_ID,version:1},resources:[{manifest,files}]});
  check(bytes.length<=5*1024*1024,'APPROVED_POMERANIAN_ZIP_TOO_LARGE');
  unpackStaticPackage(bytes);
  const file=APPROVED_POMERANIAN_ID+'.zip';
  return {file,bytes,entry:{assetId:APPROVED_POMERANIAN_ID,version:1,kind:'module',file,bytes:bytes.length,sha256:hash(bytes),rootContentHash:manifest.contentHash,label:content.entry.label,
    tags:['builtin','prefab','playable','approved-demo','博美','白色博美','同款博美','可爱','小麦','pomeranian','dog','Mochi','跟随','follow','抚摸','pet','等待','wait'],
