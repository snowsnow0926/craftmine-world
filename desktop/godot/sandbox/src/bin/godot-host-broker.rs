...
use craftmine_godot_sandbox_probe::{broker::{Request, execute}, preflight, recovery, Result};
...
fn run() -> Result<()> {
    ...
    // Host-owned recovery pass for tasks whose broker was killed before its
    // final response. It accepts only a tasks root, never a task or a path
    // outside that root.
    if args.get(1).map(String::as_str) == Some("recover") {
        let tasks_root = args.get(2).ok_or("recover requires an absolute tasks root")?;
        let report = recovery::recover(Path::new(tasks_root))?;
        let text = serde_json::to_string(&report)?;
        if let Some(index) = args.iter().position(|value| value == "--json-out") {
            let path = args.get(index + 1).ok_or("--json-out requires a path")?;
