...
/// Recovery-journal bookkeeping for one task. `cleared` is true only when the
/// broker reached its final response and retired its own journal entry.
#[derive(Debug, Serialize)]
#[serde(rename_all="camelCase")]
...
pub fn execute(request: Request, cancel: Arc<AtomicBool>) -> Result<Response> {
    ...
    let journal_entry = crate::recovery::JournalEntry::prepared(
        &task.task_id, &request.request_id, request.operation.as_str(), journal.tasks_root(),
        &task.layout.root, &task.profile_name, task.sid(), &task.identity_nonce)?;
    let journal_path = journal.write(&journal_entry)?;
