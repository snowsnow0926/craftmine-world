...
func _ready() -> void:
	...
	player = world.get_node_or_null(combat.player_path) as CharacterBody3D
...
func _physics_process(delta: float) -> void:
	if health <= 0 or not combat.context.small_monsters_running(): return
	clock_value += delta
	...
	if combat.context.get("health") <= 0:
		aggro = false
		windup = 0.0
	...
	if stagger > 0.0:
		...
		if windup <= 0.0:
			...
			if distance < 2.05 * body_size and height_difference < 0.8 and _line_of_sight():
				combat.context.call("damage_player", attack_damage)
			attack_wait = 1.5
	elif aggro and combat.context.get("health") > 0:
	...
	if facing.length() > 0.1:
		basis = Basis(Vector3.UP, wrapf(lerp_angle(rotation.y, atan2(-facing.x, -facing.z), minf(1.0, delta * 6.0)), -PI, PI))
	_refresh_visuals()
...
func take_hit(amount: int, direction: Vector3) -> bool:
	...
	if health == 0:
		...
		combat.context.call("monster_defeated", self)
	_refresh_visuals()
	return true

func snapshot() -> Dictionary:
	return {"format": "craftmine.promo-hornling/1", "entityId": entity_id, "sourceSettings": {"maxHealth": max_health}, "settings": {"maxHealth": max_health}, "health": health, "position": [global_position.x, global_position.y, global_position.z], "yaw": rotation.y}
...
func validate_state(data: Dictionary) -> String:
	...
	if data.format != "craftmine.promo-hornling/1" or data.entityId != entity_id: return "Monster identity mismatch"
	...
	if not Contract.finite(data.position[0], combat.saved_position_min.x, combat.saved_position_max.x) or not Contract.finite(data.position[1], combat.saved_position_min.y, combat.saved_position_max.y) or not Contract.finite(data.position[2], combat.saved_position_min.z, combat.saved_position_max.z) or not Contract.finite(data.yaw, -PI - 0.001, PI + 0.001): return "Monster pose out of bounds"
	return ""
