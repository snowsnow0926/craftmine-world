...
- `yaw` 是驱动实际世界旋转的朝向标量；运动和恢复经同一入口设置真实旋转及标量。保存时避免把矩阵再次分解的跨平台浮点差异当作新朝向。缓存同时绑定实际 `global_basis`，外部节点或父节点改变旋转后重新捕获实际朝向，不掩盖这些变化。位置仍直接读取实际 `global_position`，状态字段与严格比较保持不变。
