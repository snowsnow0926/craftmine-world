...
# Legacy-sized compatibility defaults. Configure from the receiving world before reuse.
...
func validate_state(data: Dictionary) -> String:
	...
	if not _valid_saved_position_bounds():
		return "PET_STATE_BOUNDS_INVALID"
	if not _valid_world_position(data.position):
		return "PET_STATE_POSITION_INVALID"
...
# Source-configured receiving-world limits; these are not saved gameplay fields.
# Two millimeters matches the contact inset used by validate_restored_state.
# Accept contact roundoff without clamping or rewriting the original position.
func _valid_saved_position_bounds() -> bool:
	for axis in 3:
		if not _finite(saved_position_min[axis], -100000, 100000) or not _finite(saved_position_max[axis], -100000, 100000) or saved_position_min[axis] >= saved_position_max[axis]:
			return false
	return true

func _valid_world_position(position_value: Array) -> bool:
	for axis in 3:
		var tolerance := 0.002 if axis == 1 else 0.0
		if not _finite(position_value[axis], -100000, 100000) or not _finite(position_value[axis], saved_position_min[axis] - tolerance, saved_position_max[axis] + tolerance):
			return false
	return true
