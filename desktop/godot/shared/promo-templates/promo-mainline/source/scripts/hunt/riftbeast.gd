...
func _ready() -> void:
	arena = get_parent()
	hunter = arena.get_parent().get_node("Player") as CharacterBody3D
...
func start_battle() -> void:
	health = MAX_HP
	global_position = Vector3(21, 0.04, 9)
...
func _face(direction: Vector3, delta: float, speed: float = 4.0) -> void:
	rotation.y = wrapf(lerp_angle(rotation.y, atan2(-direction.x, -direction.z), minf(1.0, speed * delta)), -PI, PI)
