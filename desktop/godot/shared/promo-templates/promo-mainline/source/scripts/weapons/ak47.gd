...
const RifleModel = preload("res://assets/blender/ak47.glb")
...
func _ready() -> void:
	...
	old_encounter = world.get_node("MonsterEncounter") as Node3D
...
func _input_allowed() -> bool:
	return is_instance_valid(player) and player.get("input_enabled") == true and world.get("ready_for_play") == true and not get_tree().paused

func _alive_and_playing() -> bool:
	if not _input_allowed(): return false
	if duel.get("status") == "active": return bool(duel.call("combat_running"))
	if duel.get("status") == "failed": return false
	if duel.get("status") == "ready" and int(old_encounter.get("health")) <= 0: return false
	return not bool(player.call("movement_locked"))

func _hands_free() -> bool:
	return duel.get("status") != "active" or duel.get("action") == "none"
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventKey:
		...
		if code == KEY_J:
			...
			get_viewport().set_input_as_handled()
		elif code in [KEY_LEFT, KEY_RIGHT, KEY_UP, KEY_DOWN] and duel.get("status") == "ready":
			get_viewport().set_input_as_handled()
...
func _physics_process(delta: float) -> void:
	...
	if not _alive_and_playing():
		...
		if duel.get("status") == "failed": reload_remaining = 0.0
		return
...
func _fire() -> void:
	...
	if not hit.is_empty():
		...
		if target == duel.get_node("Riftbeast") and duel.get("status") == "active":
		...
		elif target != null and target.is_in_group("grove_hostiles") and duel.get("status") == "ready":
			successful = bool(target.call("take_hit", DAMAGE, direction))
		if successful:
			hit_time = 0.15
			duel.call("_spark", end)
...
func _process(delta: float) -> void:
	...
	if equipped and duel.get("status") == "ready" and _alive_and_playing():
...
func _update_visuals(delta: float) -> void:
	...
	var blade: Node3D = duel.get("blade") as Node3D
	if blade != null: blade.visible = not equipped and duel.get("status") != "ready"
...
func _update_hud() -> void:
	...
	if equipped:
		ammo_label.text = "AK47   %02d / 30   ·   备用弹药 ∞\n按住 J／左键 连射 · R 换弹\n按住 K／右键 瞄准 · 3 切回重刃" % rounds
		if reload_remaining > 0.0: ammo_label.text += "\n换弹 %.1f 秒" % reload_remaining
		elif duel.get("status") == "failed": ammo_label.text += "\nH 重新挑战 · B 返回树林"
		elif duel.get("status") == "active" and duel.get("duel_paused"): ammo_label.text += "\n已暂停 · Esc 继续"
		elif notice_time > 0.0: ammo_label.text += "\n" + notice
		elif duel.get("status") == "ready" and int(old_encounter.get("health")) <= 0: ammo_label.text += "\n先按 R 恢复生命"
	else:
		ammo_label.text = "2 装备 AK47 · 3 重刃"

func snapshot() -> Dictionary:
	return {"format": "craftmine.ak47/1", "entityId": entity_id, "settings": {"magazineSize": MAGAZINE_SIZE}, "sourceSettings": {"magazineSize": MAGAZINE_SIZE}, "equipped": equipped, "rounds": rounds, "shots": shots, "cooldown": cooldown, "reloadRemaining": reload_remaining}

func validate_state(data: Dictionary) -> String:
	if not Contract.fields(data, ["format", "entityId", "settings", "sourceSettings", "equipped", "rounds", "shots", "cooldown", "reloadRemaining"]): return "Invalid AK47 fields"
	if data.format != "craftmine.ak47/1" or data.entityId != entity_id: return "AK47 identity mismatch"
	...
	if not data.equipped is bool or not Contract.integer(data.rounds, 0, MAGAZINE_SIZE): return "Invalid AK47 ammunition"
