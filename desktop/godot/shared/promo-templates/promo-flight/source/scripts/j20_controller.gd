...
func reset_flight(air_start := false) -> void:
	position = Vector3(0, 450, -900) if air_start else Vector3(0, REST_HEIGHT, 180)
	throttle = 0.7 if air_start else 0.0
	airspeed = 150.0 if air_start else 0.0
	pitch = 0.0
	heading = 0.0
	bank = 0.0
	rotation = Vector3.ZERO
	gear_down = not air_start
	grounded = not air_start
	crashed = false
	has_flown = air_start
	vertical_speed = 0.0
	velocity = Vector3(0, 0, -airspeed)
	flight_seconds = 0.0
	message = "自由飞行：A / D 压坡度转弯，↑ / ↓ 俯仰" if air_start else "按住 W 加油门；240 km/h 后按 ↓ 抬头起飞"
...
func on_runway() -> bool:
	return absf(position.x) < 28.0 and position.z > -1745.0 and position.z < 445.0
