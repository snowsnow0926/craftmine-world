...
W1 has component integration and native offscreen lifecycle coverage. W2 now connects PI world inspection, resource reads, draft patches, background checks, independent model review, request assertions and player application. The original PI projects, sessions, chat, model settings and work panels remain available. Visible desktop composition, polished onboarding/icons and installer acceptance remain pending. Full native Agent creation, cross-session library reuse, compaction and gameplay composition must not be reported as completed based on fixture probes.
