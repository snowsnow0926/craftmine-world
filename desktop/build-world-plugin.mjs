...
for (const file of ['manifest.json','main.cjs','core-client.cjs','world-tools.cjs','verification-jobs.cjs','review-jobs.cjs','applications.cjs','host-requests.cjs','context-review.cjs','workbench-service.cjs','godot-executor.cjs']) await fs.copyFile(path.join(source,file),path.join(output,file));
