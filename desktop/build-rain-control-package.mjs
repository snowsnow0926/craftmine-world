...
export function buildRainControlPackage({repository,root=path.join(repository,'desktop/godot/components/rain-control'),version=1}){
  ...
  const content={assetId:RAIN_CONTROL_ID,version,kind:'module',
  ...
  const bytes=packStaticPackage({root:{id:RAIN_CONTROL_ID,version},resources:[{manifest,files}]});
  check(bytes.length<=5*1024*1024,'RAIN_PACKAGE_TOO_LARGE');unpackStaticPackage(bytes);
  const file=RAIN_CONTROL_ID+(version===1?'':'.v'+version)+'.zip';
  return {file,bytes,entry:{assetId:RAIN_CONTROL_ID,version,kind:'module',file,bytes:bytes.length,sha256:hash(bytes),rootContentHash:manifest.contentHash,label:content.entry.label,
    tags:[...(version===2?['compatibility-version']:[]),'builtin','playable','reusable-world-content','rain','weather','reverse','suspend','控雨','凝雨','悬停','倒流','逆流','雨的逆序','技能'],
