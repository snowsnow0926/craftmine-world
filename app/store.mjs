...
export class ProjectStore {
  constructor(root) {
    ...
    if (!fs.existsSync(this.file)) {
      ...
      atomicJSON(this.file, { format: 'craftmine.project/1', current: build.id, snapshot: clone(INITIAL_SNAPSHOT), candidate: null, applying: null, tasks: [], messages: [], history: [{ id: build.id, summary: '空白世界', time: Date.now() }] });
    }
  ...
  commit(transactionId, snapshot) {
    ...
    const latest = validateSnapshot(snapshot); this.readBuild(tx.candidate);
    this.change(d => {
      ...
      if (task) { task.status = 'applied'; task.logs.push({time:Date.now(),text:'浏览器载入与绘制检查完成，场景版本和进度已保存。'}); }
