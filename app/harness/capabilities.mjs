...
export function capabilitiesCatalog({ extensions = [] } = {}) {
  return {
    ...
    extensions: extensions.map(extension => ({ ...extension, commands: (extension.commands || []).map(command => ({ ...command })) })),
    systems: systemList(),
...
export function capabilitiesText({ extensions = [] } = {}) {
  const catalog = capabilitiesCatalog({ extensions });
  const lines = [
    ...
    ...(catalog.extensions.length ? [
      '已装载扩展（玩法模块可以直接发出这些命令，但必须在 requires 里写 ext:ID@版本，并且声明该命令要求的权限）：',
      ...catalog.extensions.map(extension => `- ${extension.id}@${extension.version}「${extension.name}」：${(extension.commands || []).map(command => `${command.type}（权限 ${command.permission}；字段 ${(command.fields || []).map(field => field.name).join(',')}）`).join('、')}`),
    ] : []),
  ];
  return lines.join('\n');
}
