...
export class AssetPanel {
  constructor(host){
    ...
    $('asset-target').onchange=()=>{this.objectId=$('asset-target').value;this.list();};$('asset-unbind').onclick=()=>this.bind(null,null);
  ...
  async importFile(){
    ...
    try{
      ...
      const result=await this.host.api('/api/assets/import',input);await this.host.refresh();$('asset-import-status').textContent=`素材已保存为 v${result.version}，旧版本继续保留。${this.host.active()?'':'若原世界缺少这个文件，刷新页面即可重新载入。'}`;await this.open(result.id,result.version);
  ...
  render(project){this.project=project;const key=JSON.stringify([project.assets||[],project.current,project.candidate?.id,project.applying?.id,project.tasks.at(-1)?.status,this.host.active()?.version]);if(key!==this.key){this.key=key;this.list();}}
  list(){
    const scene=this.host.active()?.build.scene,objects=scene?.objects||[],target=$('asset-target');target.replaceChildren(node('option','选择世界中的对象'));target.firstChild.value='';for(const object of objects){const option=node('option',object.name+' · '+object.id);option.value=object.id;target.append(option);}if(!objects.some(o=>o.id===this.objectId))this.objectId='';target.value=this.objectId;
    const selected=objects.find(o=>o.id===this.objectId),blocked=this.bindingBusy||!!this.project?.candidate||!!this.project?.applying||this.project?.tasks.some(t=>['running','validating','cancelling'].includes(t.status));$('asset-unbind').disabled=!selected?.appearance||blocked;$('asset-binding-info').textContent=selected?(selected.appearance?`当前使用素材 v${selected.appearance.asset.version}。更换后保留碰撞和已有玩法。`:'当前使用生成的几何外观。替换会先生成可预览的候选。'):'选好对象后，在素材卡片上选择版本并准备替换。';
    ...
    for(const entry of [...matches].reverse()){
      ...
      const next=button('导入新版本',()=>this.choose(entry));next.disabled=this.busy;actions.append(select,button('预览',()=>this.open(entry.id,Number(select.value))),next,button('导出素材包',async()=>{const asset=await this.host.api('/api/assets/read?id='+entry.id+'&version='+select.value);this.host.download(asset,`${entry.name}-v${select.value}.craftasset.json`);}));const bind=button('用于所选对象',()=>this.bind(entry.id,Number(select.value)));bind.disabled=!selected||blocked;actions.append(bind);card.append(info,actions);out.append(card);
    }
  }
  async bind(id,version){
    if(this.bindingBusy)return;this.bindingBusy=true;this.list();
    try{const active=this.host.active();if(!active||!this.objectId)throw Error('请先选择世界中的对象');await this.host.api('/api/assets/bind',{version:active.version,objectId:this.objectId,assetId:id,assetVersion:version});await this.host.refresh();this.host.staged();}catch(error){this.host.toast(error.message);}finally{this.bindingBusy=false;this.list();}
  }
