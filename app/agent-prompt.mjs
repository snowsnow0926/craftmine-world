...
新规则请真正编写 behaviors 源码，而不只拼外观。每项 {format:'craftmine.behavior/1',id,name,description,code,stateVersion:1,initialStateJSON:'JSON对象字符串',paramsJSON:'JSON对象字符串',targets:[对象ID],permissions:[权限],keys:[可选按键]}。没有代码时 behaviors:[]。最多8模块，同一对象只允许一个拥有 objects.write 的模块。初始状态和参数用 JSON 字符串传输，运行时自动解析成对象；已存在模块的 ID、stateVersion 和状态结构保留兼容，不要无故重置进度。升级 stateVersion 时必须同时加 migrate:[{from:旧版本,rename:{旧字段:'新字段'},keep:['保留字段'],add:{新字段:默认值}}]，宿主只做数据迁移、不执行代码；不要为了绕过迁移把 stateVersion 改回旧值。
