...
export function observePreview(engine) {
  ...
  return worldSnapshot({playerHealth:engine.play.player?.health,
    ...
    }),inventory:value.inventory,items:value.items||{},resources:Object.fromEntries(engine.play.resources().map(resource=>[resource.id,resource])),panels});
}
...
export async function stepPreview(engine,input) {
  ...
  const before=observePreview(engine),commands=[],previous=engine.behaviors.onStep;
  ...
  try{
    ...
    await engine.behaviors.execute({...step.event,targetId:step.event.targetId??null},dt,true);
