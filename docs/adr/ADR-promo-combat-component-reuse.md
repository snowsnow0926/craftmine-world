...
The v1 release also refuses known overlapping player combat interfaces. For the same receiving world and player, the original `grove-combat/1`, `great-hunt/1`, `ak47/1` component formats and the supported `combat-vitals-state/1` player-vitals group produce `PROMO_EXISTING_COMBAT_ADAPTATION_REQUIRED`. Detection reads their existing snapshot/interface; it does not migrate, remove, rename or reset them. Unknown node names or unknown formats are not classified as conflicts. The new family protocol is distinct and never conflicts with its own shared context. Existing original promotional combat should be edited in place; independent packages are for compatible worlds without these overlapping systems.
