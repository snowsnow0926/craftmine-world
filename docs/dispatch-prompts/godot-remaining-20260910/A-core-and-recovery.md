...
8. 补长期工程存储的累计计量与配额：源码历史、资源 blob、构建历史分别核算；回收须核对正式世界、候选、草稿、作品固定版本和备份引用，不能误删仍被引用的数据。你统筹回收作业与条件，M/H 提供受保护引用，N 执行合格素材正文回收，M 执行合格 Git 回收，其他模块不自行删同一份数据；B 仅处理执行任务临时资源。旧不可变修订和资产 API 在后端切换后仅作兼容适配，不能继续形成第二套历史或素材事实。

验收覆盖 A05、A09–A12、A14 的核心部分。使用真实数据库/文件和进程重启，验证跨世界、累计用量、过期接续、回执不确定性；不得只测 happy path。向 C/D/E/H/L/I 提供完整请求/响应及错误示例。

新增配套计划的接口分工：M 独占真实 Git 内容历史与共享 ContentRef/锁文件规范，N 独占素材正文多版本与索引。你负责将现有 godot_projects/builds/jobs/applications 接到它们，SQLite 只保留部署、任务、回执、进度和必要索引，不能继续扩出第二套平行源码历史。长期空间计量通过 M 的受保护 Git 引用、N 的正文可达性和 H 的备份边界计算。迁移、Git 引用推进/DB 提交/实例提升的崩溃窗口由你和 M/D 联合验证。
