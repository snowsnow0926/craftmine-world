...
新增配套计划的接口分工：模型工具增加 M 的历史查询、准确内容引用、分支/差异与恢复提案，以及 N/H 的固定资源检索、安装/升级提案和引用读取。操作携带宿主绑定的 OperationContext，模型不能自行切目标分支/世界；未知资源、兼容性和通过状态不能猜填。允许自然语言区分只改这一个实例、保存变体和升级所有选定实例。Git 写入仍经 M/A，不能向模型暴露任意 git/shell。
