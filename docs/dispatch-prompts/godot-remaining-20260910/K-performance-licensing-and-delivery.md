...
新增配套计划的接口分工：发行清单还需随包固定 Git、M 的配置隔离和源码历史恢复工具、N 的资源库/预览及完整依赖，分别核对实际许可。测 Git 历史/diff 和资产搜索/预览 P50/P95、冷暖缓存、文件/资源规模；1千/1万等是待测档位。首发包含 VM0–VM2、AL0–AL3 及必要 AL5 备份保护/声明，同包验证联合自动门与内容恢复，不能仅交旧版 Godot 包。
