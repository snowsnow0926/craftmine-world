...
开发使用 `codex/pi-client-20260909` 独立分支与工作树；本批已合并回本地 `master`，保留上游导入与各项修复的提交，合并后原项目 274 项回归全部通过。没有推送远程。旧网页服务、个人模型配置和用户世界没有被迁移或覆盖。

代码提交包括 `99acbe5`（Rust 服务与插件）、`c619d7b`（Rust 世界存储）和 `57f2c6d`（完整桌面外壳）。程序目录为 `desktop/build/windows-preview`；其构建来源和文件校验值见 [构建记录](evidence/windows-batch-01/build-record.json)。

开发分支与 Git 工作树登记已清理，但 `D:/Craftmine World-worktrees/pi-client-20260909` 仍有少量文件残留。Git 初次删除报告目录非空，后续原生递归清理被自动执行审核以 “blocked by policy” 拒绝；没有改用其他工具绕过。程序和原始证据已提前移入主项目，后续开发应创建新的独立工作树，不复用或自动重试删除这个残留目录。
