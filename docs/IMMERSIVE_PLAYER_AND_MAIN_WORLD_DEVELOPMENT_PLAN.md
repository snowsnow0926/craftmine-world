...
愿望的 C / D / L 描述、检索后难度判断、模型分工与社区复用统一见 [总体方案](WISH_CDL_FRAMEWORK_AND_NORTH_STAR_PROPOSAL.md)。这些用于后台决策和研发详情，沉浸入口继续只呈现玩家要得到的结果、真实任务状态和必要选择；模型、模块或界面变化不能重置世界身份、操作回执与预算。
