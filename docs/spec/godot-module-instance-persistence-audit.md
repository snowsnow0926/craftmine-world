...
后续实现记录：`9fc9a7b2` 已加入两种受审计 wrapper 的纯源码参数捕获与单实例
patch 预览；`6dba01da` 在现有实例记录中保存原 CP0 声明，并验证重导出保全。
后者已通过真实 Core 的第二世界往返，检查明确为未配置执行器而阻断。这补齐
上文两个源码接口缺口的有限实现，不改变原始运行时复现结论，也不代替下列
真实引擎、采用和冷开验收。旧实例记录缺声明时仍明确未知。
