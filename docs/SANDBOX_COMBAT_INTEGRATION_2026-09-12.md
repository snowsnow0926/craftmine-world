...
这批先发布为源码组件原型。`sandbox-weapon` 和 `sandbox-monster` 尚未加入资源目录或 Windows 成品；内置宠物包从 v1 升到 v2 只为绑定新的托管适配器哈希，v1 冻结字节保持不变。下一步应把战斗组件做成正式安装包，补怪物攻击动画、受击效果、声音和更多武器，再进行玩家自然语言新增怪物的实际测试。
