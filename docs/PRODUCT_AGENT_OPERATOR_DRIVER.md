...
显示/继续行为照常执行。隐藏页签、预览/采用期间禁用、缺失回调或界面错误会
明确拒绝，不临时解锁控件。它不发送物理输入、模拟 click 事件或申请焦点。
前后必须保持同一 worldId/buildId/instanceId 和可见会话 ID；回执还包含普通
绑定视口截图，证明当前正式世界实际可捕获。如果模型恰好自动采用了新构建或
实例已变化，会如实报身份变化，不宣称原实例未变。后续可用现有虚拟游玩命令
检查模型继续工作期间的操作体验。
