...
[沉浸游玩模式与造物主世界方案](IMMERSIVE_PLAYER_AND_MAIN_WORLD_DEVELOPMENT_PLAN.md)进一步定义工作台/沉浸两种模式和游戏内两级浮层。首版可沿用当前已验收的 Web 目标；本计划后续接入原生游戏进程时，需分别验证完整工作台浮层合成、输入归属、暂停与全屏恢复。Godot 内 HUD 和完整宿主面板不是同一项交付，不要求先把整个工作台改写成 Godot UI。
