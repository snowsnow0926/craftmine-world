...
| 运行可信边界 | 固定 adapter/bridge 已保护，原 controller/camera 未保护 | 新要求校验两份规范路径文件摘要及别名；文件缺失/改动明确 unsupported，但未核验 Player 实际绑定，保留原文件的替代脚本仍可能通过这项文件检查 |
...
已验证范围限于本批实际观察的 creation 测试 fixture，以及核心对固定形状位移/时序证据的判定。声明过新要求才增加规范路径源码摘要约束；这些文件缺失或改动、不可达 setup 或不支持的几何会明确失败，原源码保留。旧已存任务不追溯加限制。

**固定路径文件摘要不证明运行时实际绑定。** 当前未核验 Player 的实际脚本资源/继承覆盖、导出参数、碰撞形状/掩码、root.player 引用，或 camera rig 与当前 active camera 的关系。保留两份原文件但静态把 Player 绑定到替代脚本/子类，或者改 root.player 指向，不会因这项文件摘要检查而自动返回 unsupported。缺口不仅是动态替换或额外推动角色。因此不能把“保留 pin 的任意作品”称为可信物理，也不能宣称全部自定义控制器已被识别或拒绝。

没有证明玩家能从任意出生点到达机关、钥匙条件、宝箱一次奖励、冷重开、完整导航、截图外观或玩家体验。也没有重跑本批完整 LPAC→真实核心作业→自动采用的单次生产故事/安装包。实际控制器绑定与逐帧验证是下一实现切片，尚未完成；此前测试通过不能补足这个身份缺口。
