...
UI 与原生档案进入本轮 `raw/`，沿用 `createCompleteOutput` 对绝对父目录和路径链接的检查。脚本只使用固定无模型用例，清除继承的模型验收入口与包选择，浏览器和产品继续采用独立不可聚焦 headless/offscreen 档案，无实际鼠标键盘、Pointer Lock、麦克风或安装行为。模型小样本由另一条显式启用且有独立预算的入口记录，不计入本命令。
