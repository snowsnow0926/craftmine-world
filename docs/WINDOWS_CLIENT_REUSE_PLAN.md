...
W0–W5 延续上一版编号，但内容以本版为准；W0 接入探针及 W1 核心运行自动验收通过，当前推进 W2。可见桌面体验和最终安装交付继续单独跟踪。优先打通 W0–W2，先交付能创作的 Windows 测试包。
...
继续开发时，先核对当前目录是否还有其他会话写入，固定迁移基线。当前执行方式以并行派工包为准：用户分发后各组在独立工作树和独立测试目录实施，回传后由协调者验收合并；不恢复旧自动循环，不更改用户正在使用的 PI 程序或 Craftmine 世界。

## 14. 第 7 批执行状态（2026-09-09）

用户确认没有独立 Windows 测试机，先完成当前环境能做的本轮开发和验收。
第 7 批新程序、对应源码、许可与报告已交付，详见 [完整报告](WINDOWS_BATCH_07.md)
和 [集成验收矩阵](dispatch-reports/batch07/G_REPORT.md)。

W0–W3 的本机核心链路与 W4 固定组合作品的实际战斗、扩展调用、应用、存档和重启已有证据。
同一个最终 Windows 包通过固定桌面 40 项、真实模型树复用 20 项、中断恢复 12 项、
三压缩 10 项及独立持久审计 13 项、玩法组合 30 项。固定作者、真实模型作者与评审分别记录，
没有把固定战斗作品当作模型从零创作。

W5 的安装包、备份、诊断、使用说明与来源材料已交付，但干净系统首装、升级失败恢复、
卸载、跨用户凭据及物理体验仍待独立环境。无窗口性能采样不等于实际显卡或可见窗口验收。

新增累计 token 可配置与缓存前缀优化；新任务默认累计不限，其他独立预算仍保留。
过期任务恢复提示、历史已结束草稿重新挂载、通用任意代码状态迁移等限制详见报告。
重要操作持久回执、已安装草稿重新检查及扩展整步校验已完成，不再作为待开发项。
