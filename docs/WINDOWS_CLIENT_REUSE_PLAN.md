...
版本：2.0 · 2026-09-09。**本版以 PI-Desktop 为首选底座，替代 v1 的 DeepSeek Harness 桌面端方案。**

状态：研究与计划修订完成，尚未开始迁移实现；此前取消的持续开发目标和定期任务保持停止。本轮未启动 PI-Desktop、未安装插件、未请求真实模型，也未操作用户输入设备。
...
最终你使用的是名为 craftmine world / 最中幻想 的软件。打开后以游戏世界为主，旁边描述想法；Agent 读取世界和历史作品、生成实际源码、验证并交付候选，你应用后重新载入继续游玩。开发、游玩、素材三个工作区保留，代码和详细执行记录按需展开。
...
| Rust host-core | 本地 RPC、权限、进程工具、会话及设置存储 | 复用桌面基础服务，不把游戏引擎重写为 Rust |
...
| packages/craftmine-domain/ | 现有世界、存档、作品、构建与验收的 JavaScript 服务 |
...
保留现有游戏领域代码，不把几何、作品库、碰撞和存档全部迁到 Rust。Rust 专门管理 PI 的会话、设置和基础服务；Craftmine 服务独占其项目 manifest、草稿、候选、证据与玩家进度。
...
| TaskWorkspace、ProjectStore、候选预览 | 提取成可复用领域服务，维护原应用事务 |
| 作品库、素材库和经验记录 | 保留数据格式，增加 PI 检索和管理界面 |
...
| 项目 | 第一版要求 |
| --- | --- |
| 世界画面 | 先使用 PI 工作面板接口接入，再完成以世界为主的布局；不把通用代码审阅页当最终游戏首页 |
...
W0–W5 延续上一版编号，但内容以本版为准；全部仍为待实施。第一批优先打通 W0–W2，先交付能创作的 Windows 测试包。

| 阶段 | 开发内容 | 完成标准 |
| --- | --- | --- |
| W0 固定版本与接入探针 | 固定 Craftmine 与 PI 源码；记录许可／依赖；建立独立构建和插件链；核对调用身份、取消与资源加载 | 固定提交可构建；世界视图、最小工具和服务可接入；形成明确补丁清单；不使用用户个人目录 |
| W1 独立 Windows 世界客户端 | 产品名称与配置、世界面板、领域服务、数据所有权与迁移副本 | 可双击启动；能打开世界、保存并重开；无外部开发工具依赖；旧存档副本恢复正确 |
...
继续开发时，先核对当前目录是否还有其他会话写入，固定迁移基线。本版只更新计划，不恢复旧循环，不更改用户正在使用的 PI 程序或 Craftmine 世界。
