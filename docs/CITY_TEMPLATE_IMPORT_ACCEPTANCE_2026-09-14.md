...
从首次客户端启动 `02:32:06.029 UTC` 到全流程断言完成 `02:33:19.006 UTC`，约 **73 秒**。这个观测窗口包含启动、模板导入、新世界检查、步行、保存和第二次冷启动，未计入最后退出收尾；其中原生世界检查为 17.309 秒。直接复现此现成模板没有模型调用，因此模型 token 为 0。若再要求 AI 改写任务或玩法，需要另外记录实际创作消耗。
