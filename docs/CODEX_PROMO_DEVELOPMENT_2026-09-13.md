...
大小 13,712,737 字节；原始失败和成功重试均留在证据目录。
...
- A02–A06 在同一主线世界逐次采用。博美的抚摸、等待、跟随和呼唤通过真实
  游戏画布输入验证；普通怪物已触发追击和伤害，巨怪段落正在制作。
- B01 雨声修复和存档组件已采用，技能的实际输入与冷重开正在验收。
- B02 歼-20 已完成加油门、起飞、左右滚转、座舱切换、收起落架、继续飞行、
  下降和减油门实测；空中状态保存与冷重开一致。重开先显示“继续”菜单，
  需正常按回车再驾驶；菜单期间输入不应改变飞机。
- B03 已生成整城和光照修订，修订通过独立运行检查，但正式候选启动连续
  超时；旧城市与进度保留。该问题及整城步行路线尚未验收完成。
