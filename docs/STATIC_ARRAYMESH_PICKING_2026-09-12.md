...
ArrayMesh 的 AABB 可由工程覆盖，不能作为跳过近处几何的依据。显式的 mesh/instance custom AABB 会影响渲染剔除，直接拒绝；对其余预算内 ArrayMesh 验证真实数组，不用存储 AABB 假命中或排除潜在遮挡。相应代价是预算按已遍历的 ArrayMesh 计算，包括屏幕外几何；大量装饰可能仍明确回退，不承诺无限规模或逐像素拾取。

## 实际证据

锁定 Godot 4.7.2 headless 回归保持既有 44 项检查通过。新增夹具的 26 项检查覆盖索引与非索引三角、多 surface 洞口、各 surface 材质/剔面与覆盖优先级、伪造 AABB、透明/Shader/skin/脚本/动画/LOD 拒绝、单网格及总预算、未引用顶点预算。原始报告位于 `D:/cm-static-arraymesh-pick-0912/test-results/arraymesh-pick-9y5MSb/report.json` 和 `test-results/mesh-pick-ZENhs5/report.json`。
