...
- 提交：`b6fb3f1`（契约与接线）、`b00a8b8`（宿主桥）、`8a33570`（测试与文档）
...
1. **服务契约落地**：新增 S6 独占 `tool-services.cjs`，声明 `sampleLiveState/budget/executorStatus/executorEnqueue/executorCancel/isDiscussionOnly/historyMethods/libraryMethods/maxSampleAgeMs` 的 `owner/hostMethod/requiredFor`；类型错误构造期失败，未接线运行期如实报 unknown。
...
6. **能力报告暴露真实接线状态**：`godot_capability_report.services` 给出 `wired/missing` 与 owner；`HOST_METHODS`/`LOCAL_TOOLS` 的 owner 更新为 round-three 的 S1/S2/S3/S4/S5/S6，并补入 `asset.*/package.*/content.*/budget.inspect/godotExecutor.status/godotJob.usage/continue`。
...
| `tests/godot-round3/S6/{tool-services,live-and-execution-wiring,host-live-sample,host-bridge,context-recovery}.test.mjs` | 29 项新测试 |
...
| 账目 | 内容 | 证据 |
| --- | --- | --- |
| 逻辑/契约 | 126 tests，121 pass，0 fail，5 skipped（S6 新 29 项 + 继承 97 项） | `evidence/tests-logic.log` |
...
| # | 要求 | 结果 |
| --- | --- | --- |
| 1 | 交付采样/budget/作业/域服务契约；推动 S2 传参、R2 接宿主桥；主进程取得当前 world/build/base/instance/时间绑定的实时相机、装备、实体、任务状态 | **契约与传参已合入**；R2 宿主桥已实现（新增采样器 + `craftmine.godotLiveState` + 服务注册）。真实运行实例未验证（无引擎/窗口）。 |
...
| 3 | 接 S1 分支/恢复/草稿接续、S3 固定作品安装提案、S5 资源检索/预览；区分五种意图并绑定 OperationContext | 绑定与五种意图已就绪（R7 交付 + 本轮 owner/方法名核对）；**登记未完成**，工具如实报 `DEPENDENCY_NOT_WIRED`。`asset_library` 目前只有 search/read/versions，**没有预览入口**（S5 预览方法未登记）。 |
