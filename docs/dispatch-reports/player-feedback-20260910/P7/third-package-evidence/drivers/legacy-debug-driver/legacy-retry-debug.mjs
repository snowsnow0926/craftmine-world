import {attachOwnedDebugger} from 'file:///D:/cm-fb-p7-20260910/test-results/owned-debug-launch.mjs';
...
import {parameterClientArguments,inspectParameterPackage,isolatedParameterEnvironment} from 'file:///D:/cm-fb3-20260910/tests/plan-loop/parameter-client-package.mjs';
...
import {godotPersistentProgress} from 'file:///D:/cm-fb3-20260910/vendor/pi-desktop/apps/desktop/electron/main/craftmine-godot-bases-acceptance.ts';
import {OLD_BRIDGE,NEW_BRIDGE,sha,inventory,assertNativeFailure,assertRepair} from 'file:///D:/cm-fb3-20260910/tests/player-feedback/P1/legacy-retry-contract.mjs';
import {waitForInitialRuntime} from 'file:///D:/cm-fb3-20260910/tests/player-feedback/P1/legacy-runtime-ready.mjs';
...
let debuggerSession;let child,closed=true,ready=false,launch,closePromise;const pending=new Map();
...
async function start(){await verify();assert.ok(closed);closed=false;ready=false;launch={number:report.launches.length+1};report.launches.push(launch);const thisLaunch=launch;
 ...
 thisLaunch.debugger={};debuggerSession=await attachOwnedDebugger({child,executable:info.executable,exeSha256:info.identity.executableSha256,profile,output:path.join(out,'debug-launch-'+thisLaunch.number),env:env(),record:thisLaunch.debugger});
...
async function stop(){if(!launch)return;await stopDefaultClient({ended:()=>closed,quit:()=>rpc('quit',{},5000),exit:closePromise,launch,kill:()=>child.kill()});if(debuggerSession)await debuggerSession.end;assertCleanHeadlessShutdown(launch);}
