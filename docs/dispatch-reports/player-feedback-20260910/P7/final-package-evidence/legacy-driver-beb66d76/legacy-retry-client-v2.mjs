...
import {parameterClientArguments,inspectParameterPackage,isolatedParameterEnvironment} from 'file:///D:/cm-fb2-20260910/tests/plan-loop/parameter-client-package.mjs';
import {loadPackageAsar} from 'file:///D:/cm-fb2-20260910/desktop/package-asar.mjs';
import {assertCleanHeadlessShutdown} from 'file:///D:/cm-fb2-20260910/tests/player-product/shutdown-exit-audit.mjs';
import {stopDefaultClient} from 'file:///D:/cm-fb2-20260910/tests/player-product/default-client-audit.mjs';
import {godotPersistentProgress} from 'file:///D:/cm-fb2-20260910/vendor/pi-desktop/apps/desktop/electron/main/craftmine-godot-bases-acceptance.ts';
import {OLD_BRIDGE,NEW_BRIDGE,sha,inventory,assertNativeFailure,assertRepair} from 'file:///D:/cm-fb2-20260910/tests/player-feedback/P1/legacy-retry-contract.mjs';
...
const identities={externalDriver:{frozenProductionCommit:"beb66d76c00a102c755ec7aeaed59b19cf5c68a9",originalSha256:"8c100c9a0df695c4726cf30126f9fb5add2b8c235227486bccab7416bde937d5",path:fileURLToPath(import.meta.url),sha256:sha(fs.readFileSync(fileURLToPath(import.meta.url))),fixtureSourceSha256:sha(fs.readFileSync(path.join(root,'tests/player-feedback/P1/legacy-retry-fixture.mjs'))),difference:'Accept only two exact no-runtime IPC message forms within existing bounded observation poll'},commit,mode:packaged?'packaged':'development',package:info?.identity??null,
...
const observeRuntime=async()=>{try{return await rpc('godotObserve');}catch(error){if(error?.message==='No world runtime is running'||error?.message==='Error: No world runtime is running')return {};throw error;}};
...
try {
 ...
 await step('explicit product retry checks a new candidate and confirms first load',async()=>{await nav('world.creationRetry',{worldId});await until(()=>nav('world.list'),list=>{const row=list.worlds.find(w=>w.id===worldId);assert.notEqual(row?.state,'failed',JSON.stringify(row));return row?.state==='ready';},'repaired world',900000);await until(observeRuntime,x=>x.worldId===worldId&&x.instanceId,'runtime');const capture=await rpc('godotCaptureView');assert.deepEqual([capture.width,capture.height],[1280,720]);assert.ok(capture.pixelStats.sampledColors>4);report.capture=evidence(capture);});
 ...
 await step('full client restart opens the repaired build with complete saved progress',async()=>{await start();await until(()=>rpc('worldNavigationReady'),x=>x?.ready&&x.worldId===worldId,'restart navigation');await until(observeRuntime,x=>x?.worldId===worldId&&x.instanceId,'automatic reopened runtime');await nav('world.open',{id:worldId});await until(observeRuntime,x=>x.worldId===worldId&&x.instanceId,'reopened runtime');await panel('godot.runtimeSave',{freeze:true});assert.deepEqual(godotPersistentProgress(await rpc('godotSnapshot')),report.snapshot);await stop();});
 report.passed=true;
