# P8 owned stop control

The driver now consumes an exact current-turn stop file and uses its existing owned IPC to call the normal product abort. Same-turn inactive terminal metrics and strict exit are required. Stopped is not creation success. No live model, credential file, Electron process or old profile was used for these checks.
