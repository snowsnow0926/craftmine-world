...
2026-09-11 补充 [C / D / L 愿望、模型与社区总体方案](WISH_CDL_FRAMEWORK_AND_NORTH_STAR_PROPOSAL.md)：社区同时服务玩家浏览和 Agent 查询，目标是让经过验证的成果成为后续创作的起点。本计划继续负责远程权限、发布、目录和传输，不承接第二套模型任务或世界状态。
...
Agent 使用结构化目录访问与玩家页面相同的可见资源。查询可包含所需行为、内容类型、底座/引擎/运行目标和接口约束；返回紧凑摘要、不可变资源引用、依赖、参数、适用范围、检查证据、来源与版本状态，必要详情和源码按固定引用分页读取。具体字段与 CP/AL 联合冻结，不在本文把拟定接口写成已实现通道。

先按权限和明确兼容条件过滤，再按功能匹配、证据、依赖及适配成本排序，热度仅作辅助。未知兼容性不伪装为已通过；搜索命中、下载、入库、目标世界检查和成功应用分别记录。完整世界模板可以创建独立世界，抽取其中功能另需依赖与接口审查。

本地资料足够时无需查远程；社区离线仍可使用本地能力。查询不自动上传私人源码、存档或全部聊天，缓存权限与原资源一致。作者声明、社区检查和当前世界组合验证分别展示，撤回与更新沿用本计划既有规则。

检索得到候选后，由客户端按总体方案重新评估剩余 D、选择小模型配置或较强模型适配，安装与应用继续使用 CP/VM。收集成功步骤可以生成本地说明草稿，但不会自动发布、赋予 Skill 信任或授权模型训练。
