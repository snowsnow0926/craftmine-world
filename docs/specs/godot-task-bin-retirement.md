...
The executor first performs its existing source, policy and artifact validation. A private in-memory capability is captured from that specific successful broker response only after the ChildProcess `close` event confirms complete stdio closure. The host reparses the complete stdout and requires semantic equality with the previously validated exit response. Process/stream errors, stdout parse failure, changed response, extra stderr arriving after exit, oversized diagnostics, cancellation or recovery prevent retirement. Existing early stderr is retained without changing its classification. A five-second deadline after exit preserves the copies if close never arrives; a subsequent close cannot revive that expired invocation. This optional gate does not rewrite the build/core result. The capability is never reconstructed from disk or an executor ledger. Old brokers/old tasks lack this proof and retain their files.

## Acknowledgment and ordering

For import/exportWeb, the actual runtime check and artifact transfer precede `godotJob.finish`. The core must return `status: passed` for the same job and the executor ledger write must settle successfully. This existing ledger operation uses writeFile plus rename; it is not an fsync/power-loss durability guarantee (the internal acknowledgment field `ledgerFlushed` denotes that completed write promise). The helper separately requests fsync and rereads an exclusive `bin-retirement-ack.json`, containing the complete original broker receipt, closed transport diagnostics and actual core confirmation. This does not claim multi-file atomicity or protection from every power-loss scenario. An exception after the core commits but before the reply arrives is not confirmation and preserves the copies.
...
Small unit fixtures exercise import/exportWeb/preflight confirmation, fixed-only removal, full receipt and diagnostic retention, in-process idempotence, legacy absence, malformed or changed identity/pins/file sets, pending work/journal, damaged prior acknowledgment, and stopping before removal. Scripted broker/core tests drive the real executor and check that files still exist at the `godotJob.finish` boundary; pass permits retirement, failed check/refused registration/lost final reply do not. Pure in-memory ChildProcess events exercise exit-before-close, late invalid stdout, late stderr, either stream failing, and a manually advanced five-second close deadline; a clean close and existing early diagnostics are positive controls. These tests are authored fixtures, not native execution or OS-pipe evidence.
