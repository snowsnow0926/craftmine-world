...
最新集成又通过真实 Core 与最终打包工具入口的 18 项源码检查，包括只改 A、
B 与共用资源不变、Core 重启及同值不重复写入。新世界的碰撞恢复守卫通过
17 个真实引擎场景，实际 Web 候选加载也能拒绝侵入玩家保存位置的建筑。
旧世界尚未迁移；完整新客户端参数采用与游戏冷开仍未完成。
这些进展及证据边界见 [集成记录](evidence/gu3-gu4-module-integration-20260912/summary.json)。
