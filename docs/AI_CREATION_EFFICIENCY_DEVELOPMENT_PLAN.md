...
版本：1.1（衔接 C / D / L、小模型与社区复用）

编制日期：2026-09-10  
检查基线：`bcebeb1f88ad9ce4660281fd442bb0abf643c0b4`（本地 master；另有正在推进的源码与未提交计划）  
状态：开发与实验方案；本次仅编制文档，不调整玩家模型设置、不运行付费模型对照、不宣称已有提速结果。

2026-09-11 更新：结合 `0deefb07` 的文档状态补充后续分工，与 [C / D / L 愿望、模型与社区总体方案](WISH_CDL_FRAMEWORK_AND_NORTH_STAR_PROPOSAL.md)衔接。原检查基线和实验结果保持各自适用范围，本次不重跑模型或改变已交付版本。
...
第一阶段维持同一个已选模型，先研究其实际支持的思考设置和指导资料。后续按总体方案 WR2/WR4 单独验证本地小模型、快速云模型与较强模型的分工；跨模型路由在玩家允许的模型、数据和预算范围内启用。模型选择不扩大工具权限，也不授权自动发布社区内容。
...
C 是结果类别，D 是当前交付难度，L 是技术改动范围，定义统一引用 [C / D / L 总体方案](WISH_CDL_FRAMEWORK_AND_NORTH_STAR_PROPOSAL.md)。先读取世界与目标、查询本地和已授权社区，再根据实际兼容候选评估剩余工作并固定执行前 D；不按“飞机”“城市”等名词直接启动大型开发。

明确的参数面板修改由程序处理；自然语言的目标/参数提取、少量兼容候选选择与成熟模块配置，可以交给经对应任务评测的小模型。下载、依赖解析、安装、检查与应用由已有宿主工具完成；小型适配和新机制按实际需要使用更强模型。同一复杂愿望可以由不同模型处理必要子任务，但必须保持原任务、产物、操作身份和总预算。

本地 4B 为实验候选，不是已确定的生产能力。短上下文与结构化参数有助于限定任务，但 JSON 合法、工具回包成功或模型自报有把握，都不能代替实际游戏结果。具体模型版本、量化、推理程序与游戏同时运行时的资源占用需要测量；未经实验不宣称本地一定更快或更省。

工具错误先按具体原因处理，有限修复与升级沿用第 4.2 节；缺资源、版本不兼容和环境故障不交给大模型反复重写。固定本地、禁止云端或预算限制持续有效，升级不能重置用量或重复执行已完成写入。
...
总体方案第 12–13 节区分教程、参考工程、可安装模块和完整世界；找到参考工程不等于已具备可安装组件。社区结构化目录先筛选版本、目标与权限，模型再读取少量候选和必要说明；原始上传描述不自动成为可信能力声明。检索、固定版本安装和实际组合检查使用 CP/AL/CC 的同一资源身份。
...
本节“思考模式 × Skill”对照继续独立保留。[总体方案第 14 节](WISH_CDL_FRAMEWORK_AND_NORTH_STAR_PROPOSAL.md#14-如何验证小模型和社区复用是否有效)另行定义“相同模块下比较模型”“相同模型下比较复用材料”和“整批任务比较自动路由”三个实验。先用少量任务验证管线，再按预算重复运行；不同时换模型、模块、提示和验收条件后归因于单项改进。
