...
这版原会话自然纠正测试已完成：输入“新放的这棵树样子不对，我要和原来那两棵一样的橡树，位置保留”，模型经过 8 次普通请求撤销默认树，再引用既有橡树 PackedScene 新增独立实例。位置 `(-7.5,0,-2)` 保留，原两棵和已存在场景节点保持。正式检查、采用、保存、冷重开和真实选中均通过。

冷重开报告为 `test-results/desktop-native-complete-VO4Pki/adoption-22eaae4e-6183-459a-b84f-17d9a2870ec8/report.json`；清晰对比和新节点实际选中证据为同世界 `exploration-c3307bb8-5f96-4b39-b401-3236d24001a1/report.json`。本次为普通源码实例复用，新树身份变化，素材安装器登记表未新增，不应称为同一实体修改或新包登记成功。

这只能证明连续纠正能力，不能改写上一版首次复用失败或推算首次成功率；8 次与前次 14 次也不是同条件效率对照。前一版冻结成品与证据均保留。
