import {browserKey,type GameInputEvent} from '../../vendor/pi-desktop/apps/desktop/electron/main/headless-game-input';
import type {PlayIdentity} from '../../vendor/pi-desktop/apps/desktop/electron/main/headless-play-action';
...
type Access={instance:()=>PlayIdentity|null;dispatch:(identity:PlayIdentity,events:GameInputEvent[])=>Promise<any>;
...
export function createGameplayController(access:Access) {
  let active:{identity:PlayIdentity;cancelled:boolean;held:GameInputEvent[];release?:Promise<any>;work:Promise<any>}|null=null;
  ...
  return {
    ...
    async segment(requested:PlayIdentity,s:InputSegment) {
      ...
      const run={identity:{...requested},cancelled:false,held:[] as GameInputEvent[],release:undefined as Promise<any>|undefined,work:Promise.resolve(null) as Promise<any>};active=run;
      run.work=(async()=>{
        ...
        try {
          before=await evidence(run.identity,s.capture!==false);
          if(!run.cancelled){
            ...
            if(!run.cancelled)during={source:'native-wait-reply-before-release',waitReceipt};
          }
        } finally {
          try {releaseReceipt=await release(run);}
          finally{unhold();}
        }
        if((s.settleFrames??1)>0)await access.wait(s.settleFrames??1);
        after=await evidence(run.identity,s.capture!==false);
      ...
      try{return await run.work;}finally{if(active===run&&!run.held.length)active=null;}
    },
    async cancel(requested:PlayIdentity) {
      ...
      run.cancelled=true;const receipt=await release(run);return {status:'cancelling',...receipt};
    },
    async drain() {
