...
the publish job assembles the GitHub Release. The `v0.14.3` workflow temporarily
packages macOS unsigned while the signing credentials are repaired; restore the
signed/notarized macOS lane before the next stable tag. See the [release
