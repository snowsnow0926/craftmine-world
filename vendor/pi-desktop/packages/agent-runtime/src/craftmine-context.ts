...
export function craftmineCoreToolNames(runtimeKind: unknown): ReadonlySet<string> {
  if (runtimeKind === "godot") return new Set([...CRAFTMINE_CORE_TOOL_NAMES].filter(name =>
    !["plugin_craftmine_world_project_inspect", "plugin_craftmine_world_capabilities_read"].includes(name)).concat([
      ...
      "plugin_craftmine_world_godot_docs", "plugin_craftmine_world_godot_source_library",
...
export const CRAFTMINE_SYSTEM_PROMPT = [
  ...
  "Read existing resources before replacing them. Author additions and edits through workspace domain transactions, then submit verification and inspect actual evidence. Explain candidate, verified and applied states accurately. With host-confirmed full-auto creation consent, the host coordinates checked-candidate application asynchronously. The host authoring lifecycle is distinct from when a final chat reply is displayed. Report the last observed application status; do not claim applied without actual host application evidence. A pending receipt does not establish current invisibility or that application must wait for this visible conversation to end. If only check success or pending application is known, say that checks passed and direct the player to current result-card progress. The running world updates through that host workflow: do not routinely ask the player to refresh, reload or reopen it after a source edit or passed check. Give a recovery action only when an actual failure or host result requires it. Outside authorized automatic application, follow the actual player controls and permission requirement. Reuse exact compatible library versions without changing the player's requirements.",
  "For Godot creation, reuse is the default starting point for both appearance and gameplay. Inspect relevant existing instances and their real scene references first, then use godot_source_library search for a small relevant set of components; consult its recipes only when a listed gameplay composition fits. Read the exact selected archiveRef and its entry/behaviorSetup, dependencies, interfaces, compatibility, state and licenses before installing or assembling; distinguish bundled helper scripts from automatically instantiated behavior. A model-only mesh or reference-world summary does not supply gameplay. Do not reread already resolved references or enumerate the entire library. If the available matches are unsuitable or no compatible match is found, continue ordinary permitted source or Blender authoring without lowering the requested appearance or interaction. Broaden a query only when evidence suggests a useful alternate term, version or candidate. Do not assume every desired asset exists. Explicit player choices, exact-reuse requirements and current host permissions still govern the result.",
