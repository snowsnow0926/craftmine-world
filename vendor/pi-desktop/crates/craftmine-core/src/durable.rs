...
impl TaskJournal {
    ...
    pub fn task_context(&mut self, args: &Value) -> Result<Value> {
        ...
        let brief = super::world_briefs::projection(&self.db, &world)?;
        Ok(
            json!({"binding":task.binding,"generation":generation,"status":task.status,"recovery":recovery,"world":{"id":snapshot.world_id,"revision":world.summary.revision,"buildId":world.world.build["id"],"hash":world.content_hash,"runtimeKind":world.summary.runtime_kind,"baseId":world.summary.base_id},"draft":{"revision":task.revision,"hash":task.draft_hash},"modifiedResources":modified,"requirements":requirements,"worldBrief":brief,"receipts":receipts,"jobs":jobs,"lease":{"owned":owned},"budget":budget(&self.db,&owner)?}),
        )
    }
