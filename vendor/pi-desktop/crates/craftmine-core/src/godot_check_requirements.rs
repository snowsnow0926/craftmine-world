...
fn valid_creation(v:&Value)->bool {
 ...
 v.get("timeOfDay").is_none_or(|t|t==18)&&valid_duplicates(v.get("duplicates"))&&valid_harvest(v.get("harvest"))&&valid_door(v.get("doorSequence"))&&door_passage::valid_requirement(v)&&es.len()<=256&&cs.len()<=5&&(! (es.is_empty()&&cs.is_empty())||v.get("doorSequence").is_some()||v.get("harvest").is_some()||v.get("timeOfDay").is_some()||v.get("duplicates").is_some())&&es.iter().all(|e|keys(e,&["id","kind","position","rotationY","scale","color","declaredLabel","visible","solid","absent","excludeIds"])&&(e.get("id").is_some()||e.get("kind").is_some())&&e.get("id").is_none_or(|i|i.as_str().is_some_and(identifier))&&e.get("kind").is_none_or(kind)&&e.get("position").is_none_or(vec3)&&e.get("rotationY").is_none_or(|r|r.as_f64().is_some_and(|r|r.is_finite()&&r.abs()<=180.0))&&e.get("scale").is_none_or(|s|vec3(s)&&s.as_array().unwrap().iter().all(|n|n.as_f64().is_some_and(|n|n>0.0&&n<=20.0)))&&e.get("color").is_none_or(|c|c.as_str().is_some_and(|s|s.len()==7&&s.starts_with('#')&&s[1..].bytes().all(|c|c.is_ascii_hexdigit())))&&e.get("declaredLabel").is_none_or(|label|e["kind"]=="marker"&&label.as_str().is_some_and(|s|s.encode_utf16().count()<=80))&&e.get("visible").is_none_or(Value::is_boolean)&&e.get("solid").is_none_or(Value::is_boolean)&&e.get("absent").is_none_or(|a|a==true&&e["id"].as_str().is_some_and(identifier))&&e.get("excludeIds").is_none_or(|ids|ids.as_array().is_some_and(|ids|ids.len()<=256&&ids.iter().all(|id|id.as_str().is_some_and(identifier)))))&&cs.iter().all(|c|keys(c,&["kind","count"])&&kind(&c["kind"])&&c["count"].as_u64().is_some_and(|n|n<=256))
}
fn creation_matches(r:&Value,entities:&[Value])->bool {
 ...
 entities.iter().filter(|e|w.get("id").is_none_or(|id|&e["id"]==id)&&w.get("kind").is_none_or(|kind|&e["kind"]==kind)&&w["excludeIds"].as_array().is_none_or(|ids|!ids.contains(&e["id"]))&&w.get("position").is_none_or(|v|close(&e["position"],v))&&w.get("rotationY").is_none_or(|v|v.as_f64().zip(e["rotationY"].as_f64()).is_some_and(|(w,a)|((a-w+180.0).rem_euclid(360.0)-180.0).abs()<=0.005))&&w.get("scale").is_none_or(|v|close(&e["scale"],v))&&w.get("declaredLabel").is_none_or(|v|&e["parameters"]["label"]==v)&&w.get("visible").is_none_or(|v|e.get("visible")==Some(v))&&w.get("solid").is_none_or(|v|e.get("solid")==Some(v))&&w.get("color").is_none_or(|c|e["color"].as_str().is_some_and(|s|s.eq_ignore_ascii_case(c.as_str().unwrap())))).count()==1
 })
}
