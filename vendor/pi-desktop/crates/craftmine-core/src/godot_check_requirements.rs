...
pub fn evidence_matches(requirements: &Requirements, evidence: Option<&Value>, descriptor: &Value) -> bool {
    ...
    evidence.format == "craftmine.godot-check-requirements-evidence/1"
        ...
        && evidence.observations.iter().zip(["loaded", "running"]).all(|(actual, phase)| {
            ...
            if let Some(creation)=&requirements.creation {return actual.target_id.is_none() && actual.hit_flash_milliseconds.is_none() && actual.entities.as_ref().is_some_and(|entities|creation_matches(creation,entities)) && (if phase=="running" {door_trace_matches(creation,actual.door_trace.as_ref())}else{actual.door_trace.is_none()});}
            let Some(target)=&requirements.target_feedback else{return false;};
            actual.entities.is_none() && actual.door_trace.is_none() && actual.target_id.as_deref()==Some(target.target_id.as_str()) && actual.hit_flash_milliseconds.is_some_and(|value|value.is_finite()&&(value-f64::from(target.hit_flash_milliseconds)).abs()<=1e-6)
        })
}
...
fn valid_creation(v:&Value)->bool {
 if !keys(v,&["format","requestHash","entities","counts","doorSequence"])||v["format"]!="craftmine.creation-requirements/1"||!v["requestHash"].as_str().is_some_and(|s|s.len()==64&&s.bytes().all(|c|c.is_ascii_hexdigit()&&!c.is_ascii_uppercase())) {return false;}
 let (Some(es),Some(cs))=(v["entities"].as_array(),v["counts"].as_array())else{return false;};
 valid_door(v.get("doorSequence"))&&es.len()<=256&&cs.len()<=5&&(! (es.is_empty()&&cs.is_empty())||v.get("doorSequence").is_some())&&es.iter().all(|e|keys(e,&["id","kind","position","scale","color","absent","excludeIds"])&&(e.get("id").is_some()||e.get("kind").is_some())&&e.get("id").is_none_or(|i|i.as_str().is_some_and(identifier))&&e.get("kind").is_none_or(kind)&&e.get("position").is_none_or(vec3)&&e.get("scale").is_none_or(|s|vec3(s)&&s.as_array().unwrap().iter().all(|n|n.as_f64().is_some_and(|n|n>0.0&&n<=20.0)))&&e.get("color").is_none_or(|c|c.as_str().is_some_and(|s|s.len()==7&&s.starts_with('#')&&s[1..].bytes().all(|c|c.is_ascii_hexdigit())))&&e.get("absent").is_none_or(|a|a==true&&e["id"].as_str().is_some_and(identifier))&&e.get("excludeIds").is_none_or(|ids|ids.as_array().is_some_and(|ids|ids.len()<=256&&ids.iter().all(|id|id.as_str().is_some_and(identifier)))))&&cs.iter().all(|c|keys(c,&["kind","count"])&&kind(&c["kind"])&&c["count"].as_u64().is_some_and(|n|n<=256))
}
