...
const REBUILDABLE: &[(&str, &str)] = &[
    (
        "godot-import-cache",
        "regenerated by the Godot importer from the archived source blobs",
    ),
    (
        "godot-build-artifacts",
        "rebuilt by the managed build job from the archived source and the fixed toolchain",
    ),
    (
        "godot-build-cache",
    ...
    (
        "git-config",
        "local tool configuration, recreated by the Git adapter",
    ),
];
...
fn enumerate_repositories(
    ...
    let repos = db
        .prepare("SELECT repo_id,object_format FROM craftmine_content_repositories WHERE backend='git' ORDER BY repo_id")?
        .query_map([], |row| {
            Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?))
        })?
        .collect::<rusqlite::Result<Vec<_>>>()?;
    ...
    for (repo_id, object_format) in repos {
...
impl TaskJournal {
    ...
    pub fn backup_export_portable(&mut self, args: &Value) -> Result<Value> {
        ...
        let result = (|| -> Result<Value> {
            ...
            let (names, tables_snapshot, entries) = {
                ...
                for pin in &pins {
                    tx.execute(
                        "INSERT OR REPLACE INTO craftmine_backup_pins(archive_id,kind,ref,world_id,status,archive_hash,created_at,updated_at)
                         VALUES(?1,?2,?3,?4,'streaming',NULL,?5,?5)",
                        params![id, pin.kind, pin.reference, pin.world, created_at],
                    )?;
                }
                tx.execute(
            ...
            let header = json!({
                ...
                "entries": all_entries.iter().map(ContentEntry::to_json).collect::<Vec<_>>(),
            ...
            let mut footer = json!({
                ...
                "archiveHash": digest(&serde_json::to_string(&header)?),
            });
            writer.line(&footer)?;
    ...
    /// Reads the archive header and footer without touching any content body.
    pub fn backup_inspect_portable(&self, args: &Value) -> Result<Value> {
        ...
        reader.read_exact(&mut magic).context("BACKUP_ARCHIVE_TRUNCATED")?;
        ensure!(magic == MAGIC, "BACKUP_FORMAT_UNSUPPORTED");
        let header: Value = serde_json::from_str(&read_line_limited(&mut reader)?)?;
        ensure!(
        ...
        Ok(json!({
            ...
            "entries": header["entries"].as_array().map(Vec::len).unwrap_or(0),
    ...
    pub fn backup_restore_portable(&mut self, args: &Value) -> Result<Value> {
        ...
        let resolved = if target.try_exists()? {
        ...
        if in_place {
            ...
            if resolved.try_exists()? {
                    ...
                    fs::read_dir(&resolved)?.next().is_none(),
    ...
    /// restore of an archive cannot destroy anything.
    ...
    /// Startup sweep. A pin is only retained when its archive job completed;
    /// anything else was interrupted before the archive was proven, so it must
    /// not keep protecting content that no archive actually holds.
    pub fn backup_recover(&mut self) -> Result<usize> {
        let pending: Vec<String> = self
            .db
            .prepare("SELECT DISTINCT archive_id FROM craftmine_backup_pins WHERE status='streaming'")?
            .query_map([], |row| row.get::<_, String>(0))?
            .collect::<rusqlite::Result<Vec<_>>>()?;
        let mut abandoned = 0;
        for archive_id in pending {
            ...
            let status = if completed { "retained" } else { "abandoned" };
            if status == "abandoned" {
                abandoned += 1;
            }
            mark_pins(&self.db, &archive_id, status, None)?;
        }
        Ok(abandoned)
    }
}
...
fn verify_archive(path: &Path) -> Result<Value> {
    ...
    let entries = header["entries"].as_array().context("BACKUP_ENTRIES_REQUIRED")?;
    let mut seen = BTreeSet::new();
    let mut content_bytes = 0u64;
    ...
    let footer: Value = serde_json::from_str(&read_line_limited(&mut reader)?)?;
        ...
        footer["entries"].as_u64() == Some(entries.len() as u64)
            && footer["contentBytes"].as_u64() == Some(content_bytes),
        "BACKUP_FOOTER_MISMATCH"
    );
    ensure!(
        footer["archiveHash"] == digest(&serde_json::to_string(&header)?),
...
/// Restores into an empty directory. Content is staged first and only moved
/// into place after the whole stream verified, so a bad archive cannot leave a
/// half-populated data directory.
fn restore_archive(target: &Path, archive: &Path, existing: Option<&Connection>) -> Result<Value> {
    ...
    let result = (|| -> Result<Value> {
        ...
        let entries = header["entries"]
            .as_array()
            .context("BACKUP_ENTRIES_REQUIRED")?;
        ...
        for entry in entries {
            ...
            match kind {
                "domain" => {
                    let value: Value =
                        serde_json::from_slice(&fs::read(&staged_path)?).context("BACKUP_DOMAIN_INVALID")?;
                    domain = Some(value);
                }
        ...
        // Content moves into place before the domain is applied: a world is
        // only registered after its bodies are present and verified.
        ...
        apply_domain(db, &domain)?;
        let current_hash = fingerprint(db)?;
        let rebuild_required = db
...
/// Replaces every registered domain table with the archive rows. Columns are
/// matched by name so a table that gained a column since the archive was made
/// still restores with its declared defaults.
fn apply_domain(db: &Connection, domain: &Value) -> Result<()> {
    ...
    for (table, data) in map {
        ...
        let archive_columns: Vec<String> = data["columns"]
            ...
            .collect::<Result<Vec<_>>>()?;
        let live_columns = columns(&tx, table)?;
