...
impl TaskJournal {
    ...
    pub fn godot_build_latest(&mut self, args: &Value) -> Result<Value> {
        #[derive(Deserialize)]
        #[serde(rename_all = "camelCase", deny_unknown_fields)]
        struct LatestArgs { #[serde(default)] world_id: Option<String>, #[serde(default)] session_id: Option<String> }
        let args: LatestArgs = serde_json::from_value(args.clone())?;
        if let Some(world) = &args.world_id { super::godot_jobs::world_scope(&self.db, world, None)?; }
        ensure!(args.world_id.is_some() || args.session_id.is_some(), "SESSION_ID_REQUIRED");
        ...
        let job: Option<String> = self.db.query_row(
            "SELECT j.id FROM craftmine_godot_jobs j JOIN craftmine_tasks t ON t.id=j.task_id WHERE (?1 IS NULL OR j.world_id=?1) AND (?2 IS NULL OR json_extract(t.binding,'$.sessionId')=?2) ORDER BY j.created_at DESC,j.rowid DESC LIMIT 1",
            params![&args.world_id, &args.session_id], |row| row.get(0),
        ).optional()?;
        match job {
            Some(job_id) => {
                let record = super::godot_jobs::read_job(&self.db, &job_id)?;
                self.godot_build_read(&json!({"worldId":record["worldId"],"jobId":job_id}))
            },
            None => Ok(Value::Null),
        }
    }
