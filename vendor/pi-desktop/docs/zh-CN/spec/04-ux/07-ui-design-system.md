...
渲染进程里的每个滚动容器都使用同一套安静滚动条：8px、无轨道、静止时滑块
透明。只有当指针位于所属滚动区域上方，或该区域正在滚动时滑块才显示（渲染
进程会在最后一次滚动事件后的短暂保持期内给正在滚动的元素打上
...
的预留槽位（`scrollbar-gutter: stable`）保留原位，静止时只是空着。

侧边栏列表把该滚动条收窄到 6px，行获得键盘焦点时同样显示，并在所有显示
状态下使用 20% 文字色滑块。这样导航树在视觉上保持安静，交互时仍有可发现
的控件。
