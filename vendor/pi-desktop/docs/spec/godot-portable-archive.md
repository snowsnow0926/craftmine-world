...
* `backup.protected-refs` returns the live set. The core aggregates the
  `builds` pins inside `godotStorage.reclaimPlan` itself (S1), so a caller that
  omits or forges `protectedBuilds` cannot unprotect a build a retained archive
  still carries; caller pins remain additive. R6 consults `assetBlobs` /
