...
The private host derives a complete candidate snapshot from two complete progress envelopes: previous formal progress and actual freshly loaded candidate defaults. Only first-person base 0.1.0 and /body/targets, /body/interactables may add entries, keyed by nonempty unique entry.id. Every old id must still exist. Existing entry key sets and value kinds must remain compatible; all old values are copied unchanged. Collections follow actual candidate default order, so array position is not entity identity. Every other previous envelope/body field remains unchanged. Unknown envelopes/body keys, unsupported identity/version, deleted old entities and duplicate IDs fail. The actual runtime must load the full result and return it unchanged; successful derivation alone is insufficient.
