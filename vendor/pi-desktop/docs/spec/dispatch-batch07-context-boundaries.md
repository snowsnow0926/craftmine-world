...
Before compaction, merge successful `plugin_craftmine_world_godot_file_read` results into the existing read-file operation set only when an actual preceding tool call matches its returned path, source revision and manifest hash. The result must contain finite range metadata, a content hash and text. Failed, unmatched or narrated reads are ignored. This records that a path was read, possibly partially; it is not proof of complete content or freshness after a source change. Existing authoritative project facts and fresh reads remain required. This adds no mutation or model-supplied memory API.
