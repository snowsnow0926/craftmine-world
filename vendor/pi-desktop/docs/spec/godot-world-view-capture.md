...
源图和视图单边最多 8192，面积最多 16*1024*1024；view 最小 320x240 DIP。NativeImage 可能采用所属离屏合成表面的像素尺寸，不能假设与 child view 的布局尺寸仅差 DPI，也不要求两者宽高比相同。来源身份由同一 WebContents 和前后绑定校验确定，分别记录 view/source 尺寸。先检查源尺寸，再复制完整 bitmap；透明空图拒绝，真实纯黑/单色画面允许。
...
13 项无窗口/无输入测试覆盖正式暂停、可信候选ID、错误身份与不可用状态、过程中视图/尺寸/实例变更、超时及并发积累防护、纯色/空图/超限、工作台合成表面、4K/DPI与截图后缩小、原始异常消息清洗。另有独立 Electron 夹具验证真实 NativeImage 经同一 helper 返回尺寸与 PNG。成品集成还需独立正式/候选实际 WebContents 像素验收，不得用旧 headlessCapture 的临时挂接能力冒充新接口成功。
