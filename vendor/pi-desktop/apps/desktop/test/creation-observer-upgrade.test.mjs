...
const filesAt=(rev,crlf=false)=>Object.entries(SCENE_OBSERVER_RESOURCES).filter(([,resource])=>{try{execFileSync('git',['cat-file','-e',`${rev}:desktop/godot/${resource}`],{cwd:root,stdio:'ignore',windowsHide:true});return true;}catch{return false;}}).map(([name,resource])=>{let text=execFileSync('git',['show',`${rev}:desktop/godot/${resource}`],{cwd:root,encoding:'utf8',windowsHide:true}).replace(/\r\n/g,'\n');if(crlf)text=text.replace(/\n/g,'\r\n');return {path:name,sha256:sha(text),bytes:Buffer.byteLength(text)};});
