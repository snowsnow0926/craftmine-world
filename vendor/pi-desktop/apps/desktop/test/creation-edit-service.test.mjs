...
async function terminal(service){for(let i=0;i<50;i++){await new Promise(resolve=>setImmediate(resolve));const status=service.status(1,"edit-a");if(["applied","failed"].includes(status.phase))return status;}throw Error("test timeout");}
