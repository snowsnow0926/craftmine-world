...
function fixture({crlf=false,modern=false,loss=false}={}){
 ...
 return{directory,deps,capture,calls,advances,formal,source,execute:()=>createCreationSourceMigration(deps),patches:()=>patches,customCore:()=>{formal.files.find(f=>f.path==='scripts/creation_world.gd').sha256='e'.repeat(64);live=structuredClone(formal.files);},dirtyDraft:()=>{live.push(meta('scripts/unapplied.gd','custom draft'));},changeDraft:(name,text)=>{const at=live.findIndex(f=>f.path===name);if(at<0)live.push(meta(name,text));else live[at]=meta(name,text);},loseBind:()=>{failBind=true;}};
}
...
test('unknown customized core and changed migration targets are preserved and explicitly refused',async()=>{
 for(const name of ['formal','craftmine_shared/base_adapter.gd','scripts/creation_world.gd','project.godot']){const f=fixture();if(name==='formal')f.customCore();else f.changeDraft(name,'custom source');const before=structuredClone(f.source());await assert.rejects(f.execute()(ctx,f.capture),/MIGRATION_NEEDED|DRAFT_CONFLICT/);assert.equal(f.patches(),0);assert.deepEqual(f.source(),before);}
});
test('unadopted draft additions survive a trusted migration instead of blocking a new creative turn',async()=>{const f=fixture();f.dirtyDraft();const before=structuredClone(f.source().files.find(file=>file.path==='scripts/unapplied.gd'));await f.execute()(ctx,f.capture);assert.equal(f.patches(),1);assert.deepEqual(f.source().files.find(file=>file.path===before.path),before);});
test('standalone observer upgrade cannot fold an unrelated draft into its later adoption',async()=>{const f=fixture();f.dirtyDraft();f.capture.observerUpgradeOnly=true;const before=structuredClone(f.source());await assert.rejects(f.execute()(ctx,f.capture),/DRAFT_CONFLICT/);assert.deepEqual(f.source(),before);assert.equal(f.patches(),0);});
test('a concurrent draft change after the source read cannot be silently accepted after migration',async()=>{
 const f=fixture(),domain=f.deps.domain;f.deps.domain=async(method,args)=>{const result=await domain(method,args);if(method==='godotProject.patch')f.dirtyDraft();return result;};
 await assert.rejects(f.execute()(ctx,f.capture),/SOURCE_MISMATCH/);assert.equal(f.patches(),1);assert.equal(f.advances.length,0);assert.ok(f.source().files.some(file=>file.path==='scripts/unapplied.gd'));
});
test('cancellation at the write boundary never issues a patch',async()=>{
 const f=fixture();let reads=0;f.deps.assertActive=async()=>{if(++reads>=3)throw Error('USER_CANCELLED');};
 await assert.rejects(f.execute()(ctx,f.capture),/USER_CANCELLED/);assert.equal(f.patches(),0);assert.equal(f.advances.length,0);
});
