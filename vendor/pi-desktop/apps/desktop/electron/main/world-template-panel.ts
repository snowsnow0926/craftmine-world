...
const METHODS = new Set(["list", "read", "describe", "save", "status", "cancel", "export", "import"]);
...
export function createWorldTemplatePanel(options: {
  ...
  const selected = async (worldId: unknown) => {if (disposed) throw Error("WORLD_TEMPLATE_UNAVAILABLE"); if (typeof worldId !== "string" || await options.selection() !== worldId) throw Error("WORLD_TEMPLATE_WORLD_CHANGED");};
  return {
    async request(channel: string, input: any) {
      ...
      if (method === "save") {
        ...
        await selected(args.worldId); const {includePreview, ...request} = args;
    ...
    dispose() {disposed = true; imports.clear();},
  };
}
