/** Exact asset operations. Filesystem reads require the current native picker grant. */
export const ASSET_PANEL_CHANNELS = new Set([
  ...
  "asset.scan", "asset.import", "asset.preview", "asset.cancel",
]);
const fields: Record<string, string[]> = {
  ...
  "asset.scan": ["sourceRoot"],
  "asset.import": ["operationId", "sourceRoot", "sourcePath", "assetId", "version", "kind", "mediaKind", "path", "mediaType", "displayName", "source", "tags"],
  "asset.preview": ["assetId", "version"], "asset.cancel": ["assetId", "version", "detail"],
};
...
export async function requestAssetPanel(channel: string, payload: Record<string, any>, options: {
  ...
  if (["asset.preview", "asset.cancel", "asset.import"].includes(channel)
    && (!bounded(args.assetId, 80) || !Number.isSafeInteger(args.version) || args.version < 1)) throw Error("INVALID_ASSET_PANEL_REQUEST");
  if (channel === "asset.cancel" && Object.hasOwn(args, "detail") && !bounded(args.detail, 200)) throw Error("INVALID_ASSET_PANEL_REQUEST");
  const authorize = async () => {
    if (channel !== "asset.scan" && channel !== "asset.import") return;
    if (!options.authorizeAssetSource || !bounded(args.sourceRoot, 2048)
      || (channel === "asset.import" && !bounded(args.sourcePath, 2048))) throw Error("ASSET_SOURCE_GRANT_REQUIRED");
    await options.authorizeAssetSource(args.sourceRoot, channel === "asset.import" ? args.sourcePath : undefined);
  };
  ...
  await authorize();
  await current();
  const result = await options.domain("asset.request", {method: channel === "asset.import" ? "importAsset" : channel.slice(6), args});
