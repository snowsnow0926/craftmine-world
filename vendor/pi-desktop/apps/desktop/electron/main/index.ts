...
const pluginPanels = new PluginPanelHost(
  async (pluginId, channel, payload) => {
    ...
    const godotApplied=["godot.candidateApply","godot.candidateState","godot.candidateClose"].includes(channel) && (result as {status?:string})?.status==="applied";
  ...
  handle(
    ...
    async (payload: { pluginId?: string; viewId?: string }) => {
      if (payload?.pluginId === "craftmine.world" && payload?.viewId === "world") {
        await godotCandidates.closeForDeparture(); await pluginViews.prepareCraftmineForQuit();
