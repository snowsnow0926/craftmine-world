...
import { bindResumedCreationTarget } from "./resumed-creation-target";
...
async function setImmersionState(state: CraftmineImmersionState): Promise<void> {
  ...
  const window = mainWindow;
  if (state === immersionState && window) syncMainInputFocus(window);
}
...
const craftmineEnginePerformance = createCraftmineEnginePerformanceSampler({
  host: () => godotWorld, resourcesRoot: godotRoot, actualVersion: "4.7.2.stable.official.ed1daf0bf",
...
function executeNativeMenuAction(
  ...
  switch (action) {
    ...
    case "close":
      ...
      break;
    case "quit":
      // An explicit game-menu exit must not inherit close-to-tray behavior.
      // app.quit() enters the existing ordered save/shutdown lifecycle.
      app.quit();
      break;
  }
...
const craftminePanelRequest = createCraftminePanelGateway({
  ...
  resume: async (session, turnId, result) => {
    ...
    await bindResumedCreationTarget(context,result,content,{
      readContext:ctx=>plugins.requestCraftmineHost("task.context",{context:ctx}),
      bindWorld:creationTargets.bindWorld,cancel:creationTargets.cancel,
      assertCurrent:async(ctx,worldId)=>{
        if(activeTurns.get(ctx.sessionId)!==ctx.turnId||turnFinalizations.has(ctx.sessionId)
          ||await godotSelection()!==worldId)throw Error("CREATION_RECOVERY_CONTEXT_CHANGED");
      },
    });
...
async function createWindow() {
  ...
  window.webContents.on("before-input-event", (event, input) => {
    if (immersionState.active && !immersionState.blocked) {
      // F2 uses the same finite route whichever native child owns focus.
      // Prevent the downstream DOM key so the overlay cannot toggle twice.
      // Escape remains in the renderer, where menus and IME own its layering.
      const creation = input.key === "F2" ? immersionShortcut(input, immersionState.overlay !== "closed") : null;
      if (creation) { event.preventDefault(); forwardImmersionShortcut(creation); return; }
...
installHeadlessControl({
  window: () => mainWindow,
  enginePerformance: identity=>craftmineEnginePerformance(identity),
