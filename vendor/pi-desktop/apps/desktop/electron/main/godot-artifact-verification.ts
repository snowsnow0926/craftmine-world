...
export function createArtifactVerificationProgress(totalFiles:number,now=Date.now){
  ...
  return {
    ...
    operation(operation:Operation,path:string|null=current.artifactPath){if(closed)return;active=true;operationStarted=now();current={...current,operation,path:relativeLabel(path)};},
    ...
    complete(){closed=true;active=false;},
    fail(log:string[],reason:string){
      if(closed||!active)return;closed=true;const time=now();
      const errorCode=reason.match(/\b(?:GODOT_CHECK_[A-Z_]+|INVALID_GODOT_CHECK_DESCRIPTOR)\b/)?.[0]??'ARTIFACT_IO_ERROR';
      // Reserve one final slot for the existing phase failure, not one per file.
      if(log.length>62)log.splice(62);
