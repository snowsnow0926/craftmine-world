...
import {CREATION_CONTROLLER_PROFILE,validControllerEvidence,validControllerWalk,sameControllerBinding} from './creation-controller-evidence.ts';
type Vec = number[];
type Motion = {before:Vec;after:Vec;startedTick:number;finishedTick:number;targetId:string|null;controller?:{before:any;walk:any;after:any};lockedInteraction?:{op:'interact';targetId:string;physicsTick:number;doorOpen:boolean}};
export type DoorPassage = {format:'craftmine.creation-door-passage/1';doorId:string;instanceId:string;setup:'snapshot-player-only';boundsSource:'collision'|'declaration-fallback';bounds:{min:Vec;max:Vec};axis:0|2;frames:240;closed:Motion;opened?:Motion;controllerProfile?:'creation-fixed-controller/1'};
...
function motionMatches(p:DoorPassage,m:Motion|undefined,closed:boolean):boolean {
 const strong=p.controllerProfile===CREATION_CONTROLLER_PROFILE;
 if(!fields(m,['before','after','startedTick','finishedTick','targetId',...(strong?['controller',...(closed?['lockedInteraction']:[])]:[])])||!m||!vec(m.before)||!vec(m.after)||!Number.isSafeInteger(m.startedTick)||!Number.isSafeInteger(m.finishedTick)||m.startedTick<0||m.finishedTick-m.startedTick<240||m.finishedTick-m.startedTick>360)return false;
 if(strong&&(!fields(m.controller,['before','walk','after'])||!validControllerWalk(m.controller?.walk,m.controller?.before,m.controller?.after)||m.controller?.before.physicsTick!==m.startedTick||m.controller?.after.physicsTick!==m.finishedTick))return false;
 if(strong&&closed&&(!fields(m.lockedInteraction,['op','targetId','physicsTick','doorOpen'])||m.lockedInteraction?.op!=='interact'||m.lockedInteraction?.targetId!==p.doorId||m.lockedInteraction?.doorOpen!==false||!Number.isSafeInteger(m.lockedInteraction?.physicsTick)||m.lockedInteraction!.physicsTick<m.startedTick||m.lockedInteraction!.physicsTick>m.controller!.walk.startedTick))return false;
...
export function doorPassageMatches(r:CreationRequirement,p:DoorPassage|undefined,instanceId?:string):boolean {
 ...
 const strong=r.doorSequence.controllerProfile===CREATION_CONTROLLER_PROFILE;
 return !!p&&fields(p,['format','doorId','instanceId','setup','boundsSource','bounds','axis','frames','closed','opened',...(strong?['controllerProfile']:[])])&&p.controllerProfile===r.doorSequence.controllerProfile&&fields(p.bounds,['min','max'])&&typeof p.instanceId==='string'&&/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(p.instanceId)&&(!instanceId||p.instanceId===instanceId)&&p.boundsSource==='collision'&&geometry(p,r)&&motionMatches(p,p.closed,true)&&motionMatches(p,p.opened,false)&&p.opened!.startedTick>p.closed.finishedTick&&(!strong||sameControllerBinding(p.closed.controller?.after,p.opened!.controller?.before));
}
export async function measureDoorPassage(runtime:any,r:CreationRequirement,bounded:<T>(p:Promise<T>)=>Promise<T>,closed?:DoorPassage):Promise<DoorPassage>{
 ...
 const observe=async()=>{
  ...
  if(strong&&!validControllerEvidence(raw.payload.controllerEvidence))throw Error('CREATION_CONTROLLER_PROFILE_UNSUPPORTED:'+(raw.payload.controllerEvidence?.reason??'PROBE_UNAVAILABLE'));
  return raw.payload;
 };
 ...
 let lockedInteraction:Motion['lockedInteraction'];
 if(strong&&!closed){
  if(before.creation.target?.entityId!==p.doorId)throw Error('CREATION_PASSAGE_DIRECT_TARGET_MISMATCH');
  await call('interact');await call('wait',{frames:2});const observed=await observe();
  const door=observed.creation.entities?.find((e:any)=>e.id===p.doorId&&e.kind==='door');
  lockedInteraction={op:'interact',targetId:p.doorId,physicsTick:observed.creation.physicsTick,doorOpen:door?.open};
 }
 const walked=await call(strong?'controller-walk':'walk',{forward:1,right:0,frames:240});await call('wait',{frames:2});const after=await observe();
 ...
 if(strong)motion.controller={before:before.controllerEvidence,walk:walked.controllerWalk,after:after.controllerEvidence};
 if(lockedInteraction)motion.lockedInteraction=lockedInteraction;
