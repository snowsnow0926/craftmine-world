...
export type CreationEntity = {id:string;kind:string;position:number[];scale:number[];color?:string;parameters?:{label?:unknown};open?:boolean;visible?:boolean;solid?:boolean;presenceMutable?:boolean;solidMutable?:boolean;bounds?:{min:number[];max:number[]}};
...
export function validCreationRequirement(value:any):value is CreationRequirement {
 ...
 if(value.doorSequence&&(!id(value.doorSequence.doorId)||!Array.isArray(value.doorSequence.steps)||value.doorSequence.steps.length<2||value.doorSequence.steps.length>8||!value.doorSequence.steps.every(id)||new Set(value.doorSequence.steps).size!==value.doorSequence.steps.length||Object.keys(value.doorSequence).some(k=>!['doorId','steps','verifyPassage'].includes(k))||(Object.hasOwn(value.doorSequence,'verifyPassage')&&value.doorSequence.verifyPassage!==true)))return false;
...
export function freezeCreationRequirements(capture:{target:{entityId:string|null;position:number[]|null};source?:'ray'|'recent';entities?:CreationEntity[]},input:string|DirectCreationIntent):FrozenCreationRequirement {
 ...
 if(typeof input==='string'){
  ...
  else {const sequence=resolveCreationSequenceIntent(input,capture);if(!sequence)return unknown;const {steps,doorId}=sequence;r.doorSequence={doorId,steps,verifyPassage:true};r.entities.push(...[doorId,...steps].map(id=>{const e=all.find(e=>e.id===id)!;return {id,kind:e.kind,position:[...e.position],scale:[...e.scale],...(e.color?{color:e.color}:{}),...(e.kind==='marker'&&typeof e.parameters?.label==='string'?{declaredLabel:e.parameters.label}:{}),...frozenPresence(e)};}));}
 }else{
