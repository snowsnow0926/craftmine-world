...
export function freezeCreationRequirements(capture:{target:{entityId:string|null;position:number[]|null};source?:'ray'|'recent';entities?:CreationEntity[]},input:string|DirectCreationIntent):FrozenCreationRequirement {
 ...
 if(typeof input==='string'){
  ...
  else {const sequence=resolveCreationSequenceIntent(input,capture);if(!sequence)return unknown;const {steps,doorId}=sequence;r.doorSequence={doorId,steps,verifyPassage:true,controllerProfile:'creation-fixed-controller/1'};r.entities.push(...[doorId,...steps].map(id=>{const e=all.find(e=>e.id===id)!;return {id,kind:e.kind,position:[...e.position],scale:[...e.scale],...(e.color?{color:e.color}:{}),...(e.kind==='marker'&&typeof e.parameters?.label==='string'?{declaredLabel:e.parameters.label}:{}),...frozenPresence(e)};}));}
 }else{
