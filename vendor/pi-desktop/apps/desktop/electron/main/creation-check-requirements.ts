...
export function freezeCreationRequirements(capture:{target:{entityId:string|null;position:number[]|null};source?:'ray'|'recent';entities?:CreationEntity[]},input:string|DirectCreationIntent):FrozenCreationRequirement {
 ...
 if(typeof input==='string'){
  ...
  else {const sequence=/^依次触碰([A-Za-z0-9._-]+(?:、[A-Za-z0-9._-]+){1,7})后打开([A-Za-z0-9._-]+)$/.exec(text);if(!sequence)return unknown;const steps=sequence[1].split('、'),doorId=sequence[2];if(!all.some(e=>e.id===doorId&&e.kind==='door')||!steps.every(id=>all.some(e=>e.id===id&&e.kind==='marker')))return unknown;r.doorSequence={doorId,steps,verifyPassage:true,controllerProfile:'creation-fixed-controller/1'};r.entities.push(...[doorId,...steps].map(id=>{const e=all.find(e=>e.id===id)!;return {id,kind:e.kind,position:[...e.position],scale:[...e.scale]};}));}
 }else{
