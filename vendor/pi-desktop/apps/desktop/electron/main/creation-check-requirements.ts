...
export type CreationEntity = {id:string;kind:string;position:number[];scale:number[];color?:string;open?:boolean};
export type CreationRequirement = {format:'craftmine.creation-requirements/1';requestHash:string;entities:Array<{id?:string;kind?:string;position?:number[];scale?:number[];color?:string;absent?:boolean;excludeIds?:string[]}>;counts:Array<{kind:string;count:number}>;doorSequence?:{doorId:string;steps:string[]}};
...
export function validCreationRequirement(value:any):value is CreationRequirement {
 if(!value||value.format!=='craftmine.creation-requirements/1'||!/^[a-f0-9]{64}$/.test(value.requestHash)||!Array.isArray(value.entities)||!Array.isArray(value.counts)||value.entities.length>256||value.counts.length>5||value.entities.length+value.counts.length===0&&!value.doorSequence)return false;
 if(Object.keys(value).some(k=>!['format','requestHash','entities','counts','doorSequence'].includes(k)))return false;
...
export function freezeCreationRequirements(capture:{target:{entityId:string|null;position:number[]|null};entities?:CreationEntity[]},input:string|DirectCreationIntent):FrozenCreationRequirement {
 ...
 if(typeof input==='string'){
  ...
  else if(enlarge&&selected){r.entities.push({id:selected.id,kind:selected.kind,scale:selected.scale.map(n=>n*Number(enlarge[1]))});}
  ...
  else if(input.action==='modify'&&Object.keys(input.changes).length&&Object.keys(input.changes).every(k=>['scale','color'].includes(k)))r.entities.push({id:selected.id,kind:selected.kind,...input.changes});else return unknown;
 }
...
export function freezeUndoRequirements(capture:{entities?:CreationEntity[]},formalJournal:any,operationId:string):FrozenCreationRequirement {
 ...
 if(actual.length!==inverse.after.length||inverse.after.some((e:any)=>!actual.some(a=>a.id===e.id&&a.kind===e.kind&&JSON.stringify(a.position)===JSON.stringify(e.position)&&JSON.stringify(a.scale)===JSON.stringify(e.scale)&&a.color===e.color)))return unknown;
