...
export function createGodotWindowsExportService(options){
 ...
 async function run(entry){
  ...
  try{
   ...
   if(source?.format!=='craftmine.godot-export-source/1'||source.worldId!==entry.worldId||!['first-person','top-down','side-view','mining-sandbox'].includes(source.baseId)||!/^gbd-[a-f0-9]{64}$/.test(source.buildId)||!Array.isArray(source.files)||source.files.length>4096||!Number.isSafeInteger(source.sourceRevision)||source.sourceRevision<0||!Number.isSafeInteger(source.revision)||!source.snapshot||!source.sourceWorldId)fail('EXPORT_FORMAL_SOURCE_REQUIRED');
