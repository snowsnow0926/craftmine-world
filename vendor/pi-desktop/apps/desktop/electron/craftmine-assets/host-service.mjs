...
export function createAssetPreviewHost({resolveBody,runPreview=runPreviewInWorker}) {
  ...
  const active=new Map(),cancelled=new Set();let disposed=false,unconfirmedStop=null;
  ...
  return {
    preview(input){
      ...
      if(unconfirmedStop)throw unconfirmedStop;
      ...
      })().catch(error=>{if(error?.message==='ASSET_PREVIEW_WORKER_STOP_TIMEOUT')unconfirmedStop=error;throw error;}).finally(()=>{if(active.get(input.jobId)===entry)active.delete(input.jobId);});
    ...
    async dispose(){disposed=true;for(const entry of active.values())entry.controller.abort();await Promise.allSettled([...active.values()].map(e=>e.promise));cancelled.clear();if(unconfirmedStop)throw unconfirmedStop;},
  };
}
