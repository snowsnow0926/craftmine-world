export type CreationTaskStatus = {
  worldId: string; sessionId: string; taskId?: string; jobId?: string;
  phase: "idle" | "editing" | "checking" | "ready" | "applying" | "applied" | "failed" | "interrupted" | "cancelled" | "recovered";
  stage?: string; requirementStatus: "not-requested" | "pending" | "passed" | "failed" | "unsupported";
  error?: string; updatedAt?: number;
};
const phases = new Set(["idle", "editing", "checking", "ready", "applying", "applied", "failed", "interrupted", "cancelled", "recovered"]);
...
export function creationTaskPending(status: CreationTaskStatus): boolean {
  return ["editing", "checking", "applying", "recovered"].includes(status.phase);
}
...
/** One mounted observer, host facts only; unchanged stages back off and stop after 30 minutes. */
export class CreationTaskStatusObserver {
  ...
  async refresh() {
    ...
    try {
      ...
      if (!this.disposed && epoch === this.epoch) this.publish(null, true);
    }
  }
