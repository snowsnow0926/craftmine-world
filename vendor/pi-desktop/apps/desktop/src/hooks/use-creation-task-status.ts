  ...
  useEffect(() => {
    ...
    const observer = new CreationTaskStatusObserver(async () => parseCreationTaskStatus(await bridge.call("godot.creationTaskStatus", {sessionId}), sessionId),
      (status, unavailable) => {
        ...
        setResult({sessionId, status, unavailable: showUnavailable});
      }, running);
    ...
    return () => { observer.dispose(); off(); window.removeEventListener("craftmine-world-changed", refresh); window.removeEventListener("craftmine-creation-edit-status", refresh); window.removeEventListener("craftmine-viewing-session-ready", acknowledged); };
