...
export function CreationTargetContext({ controller }: { controller: ReturnType<typeof useCreationTarget> }) {
  ...
  return <div className="creation-target-context" data-creation-target={controller.loading ? "loading" : sceneObject ? "scene-object" : target ? target.surface : "none"}>
    ...
    {(controller.error || controller.capture?.reason) && <span className="creation-target-note" title={controller.error || controller.capture?.reason || undefined}>{controller.capture?.upgradeId?(zh?"此世界需要兼容更新，当前指向尚未绑定。更新会保留作品和进度；完成后请重新选中对象。仍可继续文字聊天。":"This world needs a compatibility update. No target is bound. Content and progress will be preserved; select the object again afterward. Text chat remains available."):zh ? (fromRecent?"先前选择的对象暂不可用，请选择其他结果或使用当前指向。":"指向暂不可用，可继续输入文字") : "Target unavailable; select another result or use current aim."}</span>}
