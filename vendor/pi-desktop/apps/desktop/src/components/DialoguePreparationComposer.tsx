type Props = {draft:string;queued:boolean;failed:boolean;cancelling?:boolean;onDraft:(text:string)=>void;onQueue:()=>void;onEdit:()=>void;onRetry?:()=>void};
/** Separate ownership until the real new session and runtime can receive text. */
export function DialoguePreparationComposer({draft,queued,failed,cancelling,onDraft,onQueue,onEdit,onRetry}:Props){
  return <section className="dialogue-preparation-composer no-drag" aria-label="新世界描述草稿">
    ...
    <div className="dialogue-preparation-actions">
      ...
      {failed&&onRetry&&<button type="button" disabled={cancelling} onClick={onRetry}>重试准备</button>}
